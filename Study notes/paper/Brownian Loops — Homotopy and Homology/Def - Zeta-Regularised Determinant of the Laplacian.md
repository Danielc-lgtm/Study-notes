...
> **Definition (renormalised determinant, cusped case — sketch).** On a cusped surface (finite-area but with punctures — infinite-length narrow ends), the naive heat trace $\int_X p(t,z,z)\,d\operatorname{vol}_g$ diverges because each cusp is an infinite-volume region where $p(t,z,z)$ does not decay to $0$.
>
> *Concrete check on the cusp itself.* On the model cusp $\{(x,y):0\le x<1,\,y>c\}/\langle z\mapsto z+1\rangle$, we have $\operatorname{vol} = \int_c^\infty y^{-2}\,dy \cdot 1 = 1/c$ (finite), but $\int_{\text{cusp}} p(t,z,z)\,y^{-2}\,dx\,dy$ uses the Euclidean-like density $p(t,z,z)\sim 1/(4\pi t)$ at all points and picks up an infinite contribution from the tail as $c\to\infty$ once one accounts for the Eisenstein-series pattern of the eigenfunctions.
>
