...
**Contrast with the closed case.** On a closed surface, the [[Thm - Determinant as Renormalised Loop Mass|Theorem 5.1(ii) $\kappa\to 0$ limit]] shows the same mechanism at the level of the loop mass: the $\log\kappa$ in $-\log\det_\zeta\Delta = -\operatorname{Area}(X)E + \log\kappa - \log Z_X(s) + O(\kappa)$ is exactly there to cancel $-\log(s - 1) \approx -\log\kappa$ coming from the expansion $-\log Z_X(s) = -\log Z_X'(1) - \log(s - 1) + O(s - 1)$. The two logarithms cancel and $\log Z_X'(1)$ survives — the same "divide out the simple zero" mechanism as here, dressed differently.
