...
> **In words:** hyperbolic elements are the "translations along a specific line" among the isometries of $\mathbb H^2$: pick a geodesic, slide everything along it by a fixed distance. Primitive means "not a proper power of anything else." On the surface, every loop that genuinely wraps around at least one hole (as opposed to a loop that merely circles a puncture — which can be shrunk right up to the puncture and so has no shortest representative) is homotopic to a unique shortest curve — a closed geodesic — of a definite length $\ell_\gamma$; iterating the loop $m$ times traces the geodesic $m$ times.
