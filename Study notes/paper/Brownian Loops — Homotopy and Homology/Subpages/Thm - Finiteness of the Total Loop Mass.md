...
> **Formally:** $\delta := \inf\{s > 0 : \sum_{h \in \Gamma} e^{-s\,d(z, hz)} < \infty\}$, independent of $z \in \mathbb H^2$. Equivalent descriptions: the Hausdorff dimension — a fractal-size measurement — of the limit set of $\Gamma$ on $\partial\mathbb H^2$ (the set of boundary limits of the $\Gamma$-orbit); the topological entropy — a growth-rate — of the geodesic flow (the flow that sends a point along its geodesic). $\delta = 1$ for finite-area surfaces; $\delta < 1$ for infinite-area geometrically finite ones.
