...
> **Formally:** for a closed surface with Laplace eigenvalues $0=\lambda_0<\lambda_1\le\lambda_2\le\cdots\to\infty$, the **spectral zeta function** is $\zeta_X(s):=\sum_{j\ge1}\lambda_j^{-s}$ (convergent for $\operatorname{Re}s>1$, since $\lambda_j\to\infty$ like $j$ and $\sum j^{-s}$ needs $s>1$). It equals the Mellin transform $\Gamma(s)^{-1}\int_0^\infty t^{s-1}(\operatorname{Tr}e^{-t\Delta_X}-1)\,dt$, from which $\zeta_X(s)$ meromorphically continues to all of $\mathbb{C}$ and is regular at $s=0$. Then $\log\det_\zeta\Delta_X:=-\zeta_X'(0)$.
> **In words:** you want the "product of all Laplace eigenvalues" $\prod_{j\ge1}\lambda_j$, but this product is infinite (there are infinitely many $\lambda_j$ growing to infinity). The zeta-regularised determinant is a finite, canonical stand-in for that divergent product. The trick: use $\log\prod_j\lambda_j=\sum_j\log\lambda_j$; note that formally $\sum_j\log\lambda_j=-\frac{d}{ds}\big|_{s=0}\sum_j\lambda_j^{-s}=-\zeta_X'(0)$; and *this* right-hand side, thanks to meromorphic continuation, is finite. Define $\det_\zeta\Delta:=e^{-\zeta_X'(0)}$ by fiat. It reduces to the ordinary determinant when there are only finitely many eigenvalues.
> **Concretely:** if $\Delta$ has only three eigenvalues $\lambda_1=1,\lambda_2=2,\lambda_3=3$, then $\zeta(s)=1^{-s}+2^{-s}+3^{-s}=1+2^{-s}+3^{-s}$; $\zeta'(0)=-\log 2\cdot 2^0-\log 3\cdot 3^0=-(\log 2+\log 3)=-\log 6$; so $\log\det_\zeta=-\zeta'(0)=\log 6$; giving $\det_\zeta=6=1\cdot 2\cdot 3$, the ordinary product. For infinitely many $\lambda_j$, the same recipe extends via analytic continuation. On the flat torus $T^2=\mathbb{R}^2/(2\pi\mathbb{Z})^2$, $\det_\zeta\Delta=\frac{1}{(2\pi)^2}(\text{a Jacobi-theta product})\approx 3.19$. See [[Def - Zeta-Regularised Determinant of the Laplacian]].
...
> **Formally:** on a finite-area but non-compact hyperbolic surface (one with cusps — puncture-shaped ends going off to infinity), Melrose's compactification $\bar X$ adds a boundary at infinity, with a smooth boundary defining function $x:\bar X\to[0,\infty)$ ($x=0$ exactly on the boundary). The **renormalised integral** $^{0}\!\int_X f:=\operatorname{FP}_{z=0}\int_X x^z f\,d\!\operatorname{vol}_g$ takes the divergent $\int_X f$, multiplies the integrand by $x^z$ (making it convergent for $\operatorname{Re}z>0$), analytically continues in $z$, and takes the **finite part** (constant term in the Laurent expansion) at $z=0$. The **0-trace** is $^{0}\!\operatorname{Tr}(e^{-t\Delta_X}):=\,^{0}\!\int_X p(t,z,z)\,d\!\operatorname{vol}_g$; then $\zeta^0_X(s):=\Gamma(s)^{-1}\int_0^\infty t^{s-1}(^{0}\!\operatorname{Tr}(e^{-t\Delta_X})-P)\,dt$ (with $P$ the projection onto the finite-dimensional $L^2$-null-space), and ${\det}_0\Delta_X:=e^{-(\zeta^0_X)'(0)}$. On a closed surface (no boundary, so $x\equiv 1$) this reduces to $\det_\zeta\Delta_X$.
> **In words:** on a cusped surface, the ordinary heat trace $\int_X p(t,z,z)\,d\operatorname{vol}$ diverges because the cusps contribute infinite volume of "flat" regions where $p(t,z,z)\sim 1/(4\pi t)$ but $\operatorname{vol}(\text{cusp})=\infty$. The renormalisation multiplies the integrand by $x^z$, where $x$ is a coordinate that vanishes at the cusp — this makes the integral converge for large $z$, and analytic continuation to $z=0$ picks out the "regular part" (throwing away the cusp singularity). What remains is the honest, finite spectral content — that goes into the ordinary zeta-regularisation recipe.
> **Concretely:** on the modular surface $\mathrm{PSL}(2,\mathbb{Z})\backslash\mathbb{H}^2$ (one cusp), the naive heat trace at time $t$ is $+\infty$, but the 0-trace equals a finite number that depends on $t$; taking $t\to0$ gives an *asymptotic* expansion whose constant term is $-\log(4\pi t)/2+$const. On the trivial closed sphere (no cusps), $x\equiv 1$ and the entire construction reduces to the usual $\zeta_X'(0)$ definition. **Source:** Melrose, *The Atiyah–Patodi–Singer Index Theorem*; Borthwick, *Spectral Theory of Infinite-Area Hyperbolic Surfaces*, Ch. 5. See [[Def - Zeta-Regularised Determinant of the Laplacian]].
