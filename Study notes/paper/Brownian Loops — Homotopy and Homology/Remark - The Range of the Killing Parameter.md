...
> **In words:** $\Delta_{\mathbb{H}^2}$ has no $L^2$-eigenfunctions; instead it has a *continuous* spectrum starting at $1/4$. The number $1/4$ is a geometric invariant of the hyperbolic plane — it comes from the exponential volume growth of hyperbolic balls, which contributes to the heat kernel a decay factor $e^{-\lambda_0 t} = e^{-t/4}$ at long times.
