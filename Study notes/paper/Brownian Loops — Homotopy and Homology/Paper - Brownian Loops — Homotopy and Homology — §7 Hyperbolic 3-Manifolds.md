...
> **Formally:** $\mathbb{H}^3=\{(z,y):z\in\mathbb{C},y>0\}$, metric $(|dz|^2+dy^2)/y^2$, volume $y^{-3}dA(z)\,dy$, isometries $\mathrm{PSL}(2,\mathbb{C})$; a loxodromic $\tau$ has standard form $(z,y)\mapsto(e^{L_\gamma}z,e^{\ell_\gamma}y)$, complex length $L_\gamma=\ell_\gamma+i\theta_\gamma$; the Brownian heat kernel is $p_{\mathbb{H}^3}(t,z,w)=\frac{1}{(4\pi t)^{3/2}}\frac{u}{\sinh u}e^{-t-u^2/(4t)}$, $u=d(z,w)$. See [[Def - Hyperbolic 3-Space, Kleinian Groups, and Complex Length]].
> **In words:** the 3D analogue of the hyperbolic plane; a closed geodesic now both translates and *twists*, packaged as a complex length; the heat kernel is a distance-only Gaussian with the $u/\sinh u$ curvature factor.
