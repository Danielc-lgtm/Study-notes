...
> **Formally:** $H_1(X,\mathbb{Z})=\Gamma/[\Gamma,\Gamma]\cong\mathbb{Z}^r$ ($r=2g$ closed, $r=2g+b-1$ with $b$ ends) is the abelianisation of $\Gamma$, with Hurewicz map $\Gamma\twoheadrightarrow H_1$, $\gamma\mapsto[\gamma]$, $[\gamma^m]=m[\gamma]$. A unitary character $\chi:H_1\to S^1$ lives in the dual torus $\widehat{H_1}\cong(S^1)^r$, and characters are orthonormal: $\int_{\widehat{H_1}}\chi(\beta')\overline{\chi(\beta)}\,d\chi=\mathbf 1_{\beta'=\beta}$. See [[Def - First Homology, Characters, and Finite Fourier Analysis]].
> **In words:** homology is the abelian shadow of the loop's topology; characters are its Fourier modes, and orthogonality is Fourier inversion — the tool that extracts one class from a sum over all classes.
