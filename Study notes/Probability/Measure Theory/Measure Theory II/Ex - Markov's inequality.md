...
**Markov applied to a *transformed* variable yields the whole family of moment inequalities.** Apply it to $(g-\mathbb{E}g)^2$ and you get Chebyshev; to $|g|^p$, the $p$-th moment bound; to $e^{tg}$, the exponential Chernoff bound that powers [[Thm - Cramér's Theorem|large deviations]]. The trigger-reaction: "I need to bound $\mu(\{g \text{ is extreme}\})$" → "apply Markov to a well-chosen non-negative function of $g$." The corollary $\int f=0\Rightarrow f=0$ a.e. is itself a workhorse — it is how one proves two functions are equal a.e. (show their difference integrates to $0$ in absolute value).
