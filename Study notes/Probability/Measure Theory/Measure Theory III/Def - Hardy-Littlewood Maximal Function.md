...
$f^*$ is measurable (each average $x\mapsto\fint_{B(x,r)}|f|$ is continuous, and $f^*$ is a supremum of them). It satisfies the **maximal inequality**: for $f\in L^1(\mathbb{R}^n)$ and every $a>0$,
...
For $f$ **continuous** at $x$, $\fint_{B(x,r)}|f|\to|f(x)|$ as $r\to0$, so the *small-radius* averages behave well; $f^*$ captures instead the *worst* radius.
...
> The maximal inequality is the engine of the [[Thm - Lebesgue Differentiation Theorem|Lebesgue differentiation theorem]]: $\fint_{B(x,r)}f\to f(x)$ for a.e. $x$, for every locally integrable $f$.
