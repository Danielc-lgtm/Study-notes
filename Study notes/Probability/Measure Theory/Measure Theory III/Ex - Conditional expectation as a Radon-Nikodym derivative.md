...
The proof breaks into three steps, one per sub-part. Step 1 (part a) verifies $\nu(A) = \int_A X\,d\mathbb{P}$ is a finite measure on $\mathcal{G}$ (MCT for $\sigma$-additivity, $\nu(\Omega) = \mathbb{E}X < \infty$) and that $\nu \ll \mathbb{P}|_{\mathcal{G}}$; Step 2 (part b) applies Radon-Nikodym *on the sub-$\sigma$-algebra* $\mathcal{G}$ to extract a $\mathcal{G}$-measurable density $Y$; Step 3 (part c) reads the two properties of $Y$ ($\mathcal{G}$-measurable + integral-matching on $\mathcal{G}$) as the defining characterisation of $\mathbb{E}[X\mid \mathcal{G}]$, and extends to signed $X$ by $X = X^+ - X^-$. The non-obvious move is in Step 2 — applying Radon-Nikodym *on $\mathcal{G}$ rather than on $\mathcal{F}$* is what makes the output automatically $\mathcal{G}$-measurable, which is the defining requirement of conditional expectation.
