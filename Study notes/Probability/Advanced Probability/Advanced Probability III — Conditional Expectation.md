...
- **[[Thm - Existence and Uniqueness of Conditional Expectation]]**
	- $\mathbb{E}[X\mid\mathcal{G}]$ exists (for $X\in L^1$) and is a.s. unique. Two constructions: [[Thm - Radon-Nikodym Theorem|Radon–Nikodym]] derivative on $\mathcal{G}$, or [[Thm - Conditional Expectation as L2 Projection|L²-projection]]. Uniqueness: a $\mathcal{G}$-measurable function with all $\mathcal{G}$-integrals zero is zero.
...
> [!tip] Unlocked: Martingales and Discrete-Time Stochastic Processes *(from Advanced Probability IV)*
> A **martingale** with respect to a [[Def - Filtration|filtration]] $(\mathcal{F}_n)$ is exactly an adapted sequence $(X_n)$ satisfying $\mathbb{E}[X_{n+1}\mid\mathcal{F}_n]=X_n$ — the conditional-expectation identity in motion. The tower property *is* the consistency of this definition (a martingale is its own best predictor at every coarser time), and conditional Jensen makes $|M_n|^p$ a submartingale, the input to Doob's maximal inequality and to the $L^p$-convergence theorems. Without §3.1 there is no theory of [[Def - Martingale|martingales]], and without martingales there is no continuous-time stochastic process — see [[Advanced Probability IV — Martingales in Discrete Time]].

> [!tip] Unlocked: Markov Chain Theory *(from Stochastic Processes)*
> The **Markov property** "$\mathbb{E}[f(X_{n+1})\mid X_1,\dots,X_n] = \mathbb{E}[f(X_{n+1})\mid X_n]$" is a conditional-expectation identity asserting that the conditional expectation of the future depends on the entire past only through the present. The **Chapman–Kolmogorov equations** $p^{m+n}(x,A) = \int p^m(x,dy)\,p^n(y,A)$ are an instance of the **tower property** applied to a Markov chain's $n$-step transition kernels. The whole theory of Markov chains (recurrence, ergodicity, hitting times, stationary distributions) is the calculus of conditional expectation specialised to [[Def - Filtration|filtrations]] generated by the chain itself.
...
> [!tip] Unlocked: Best Linear Predictor and the Kalman Filter *(from Statistics and Signal Processing)*
> The $L^2$-projection view identifies $\mathbb{E}[Y\mid X]$ as the **best mean-square predictor** of $Y$ from $X$ — the regression function. When the linear span of $X$ already exhausts $L^2(\sigma(X))$ (as for jointly Gaussian data), the conditional expectation is *linear* in $X$, giving the **best linear unbiased predictor** of classical statistics. Iterating this projection along a filtration of observations yields the **Kalman filter**: at each step, update the best predictor of the hidden state by orthogonally projecting onto the new measurement's added $L^2$-direction. The Kalman recursion is the law of iterated projections — exactly the tower property in $L^2$ — and is the most-used algorithm in signal processing, control, and navigation.
>
> The **law of total variance** $\mathrm{Var}(Y) = \mathbb{E}[\mathrm{Var}(Y\mid X)] + \mathrm{Var}(\mathbb{E}[Y\mid X])$ is the Pythagorean theorem applied to the projection, decomposing total variance into *unexplained* (residual within each $X$-level) and *explained* (variation of the predictor across $X$-levels) — this is the input to ANOVA, to $R^2$ in regression, and to Rao–Blackwellisation in statistical estimation.

> [!tip] Unlocked: Bayesian Posterior and Information Theory *(from Bayesian Inference)*
> The conditional-density form $\mathbb{E}[h(\theta)\mid X=x] = \int h(\theta) f_{\theta\mid X}(\theta\mid x)\,d\theta$ where $f_{\theta\mid X} = f_{\theta,X}/f_X$ is exactly **Bayes's theorem**: the posterior is the joint over the marginal. Bayesian inference is the calculus of this conditional expectation as new data $X$ accumulates, and conjugate priors are the cases where the conditional family is closed (the Gaussian-conditional-on-Gaussian case being the prototype). The **conditional entropy** $H(Y\mid X) = -\mathbb{E}[\log f_{Y\mid X}(Y\mid X)]$ is the information-theoretic refinement, measuring the residual uncertainty in $Y$ after observing $X$; the chain rule $H(X,Y) = H(X) + H(Y\mid X)$ is the entropy version of the tower property.
