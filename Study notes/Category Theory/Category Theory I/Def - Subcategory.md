...
**$\mathbf{Ab} \subseteq \mathbf{Grp}$ is full.** The [[Def - Abelian Group|abelian groups]] form a full subcategory of [[Def - Group|groups]]: every group homomorphism between two abelian [[Def - Group|groups]] is retained, so $\mathbf{Ab}(A, B) = \mathbf{Grp}(A, B)$ for abelian $A, B$. The inclusion is fully faithful. This is the standard full-subcategory example.
