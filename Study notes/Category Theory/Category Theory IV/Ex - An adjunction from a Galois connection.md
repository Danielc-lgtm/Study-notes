...
>
> **Example.** Let $P = Q = $ the power set of a set $X$, ordered by inclusion, and let $R\subseteq X\times Y$ be a relation. Define $f(A) = \{y : \forall a\in A,\ aRy\}$ and $g(B) = \{x : \forall b\in B,\ xRb\}$. Then $f(A)\subseteq B\iff A\subseteq g(B)$ (both say $A\times B\subseteq R$), so $f\dashv g$ is a Galois connection; $gf$ is the closure operator whose closed sets are the "Galois-closed" sets, the engine of **formal concept analysis** and of the Galois correspondence of field theory ($f = $ "fixed field of", $g = $ "automorphisms fixing").
