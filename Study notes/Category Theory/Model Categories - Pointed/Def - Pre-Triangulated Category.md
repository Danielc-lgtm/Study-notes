...
The aim is to write down, as a self-contained list of axioms, exactly the structure that $\mathrm{Ho}(\mathcal{C})$ carries by virtue of the constructions in this chapter — so that one can later reason about "[[Def - Homotopy|homotopy]] categories of pointed model categories" without ever returning to the model category. This is the standard move that produced abelian categories from [[Def - Module|module]] categories and triangulated categories from derived categories: distill the formal properties, forget the construction. A pre-triangulated category is the distillation for the **pointed, not-yet-stable** case; a [[Def - Cofiber and Fiber Sequence|triangulated category]] (next chapter) is the distillation once suspension is invertible.
