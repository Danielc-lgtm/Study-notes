...
The mechanism is the cleanest in the whole chapter. **In the free unbiased monoidal category, a formal expression is just a rooted planar tree whose leaves are the objects in order, and a canonical map is a way of reshaping one tree into another; coherence holds because the As [[Def - Operad|operad]] has exactly one operation of each arity, so any two reshapings with the same leaf sequence are the same operation.**
