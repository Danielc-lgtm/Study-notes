...
In the category $\mathbf{Top}$ of topological spaces and continuous maps, the subspace topology realizes the **equalizer / limit** of a particular diagram. More precisely, the inclusion $\iota : Y \to X$ with $Y$ given the subspace topology has the following universal property: it is the **initial topology** induced by the single map $\iota$ to $X$. The initial topology on a set $Y$ induced by a family of functions $\{f_\alpha : Y \to X_\alpha\}$ to topological spaces $X_\alpha$ is the *coarsest* topology making every $f_\alpha$ continuous; with one map $\iota$ and one space $X$, it specializes to the subspace topology.

This is the same universal-property machinery that, with the family of all projections $\pi_\alpha : \prod X_\alpha \to X_\alpha$, gives the **product topology**, and the dual machinery — *final* topology, coarsest making maps *out of* a space continuous — gives the **quotient topology**. The subspace, product, and quotient topologies are three instances of the same construction: take an existing space (or family) and produce the *unique* topology on a new set that makes a given map (or family of maps) continuous in the appropriate direction.
