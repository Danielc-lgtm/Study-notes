...
There are two complementary categorical formulations of a topological space. Both are illuminating, and the gap between them is the entire content of "pointless topology".

**A topological space is an object of the category $\mathbf{Top}$.** The category $\mathbf{Top}$ has topological spaces as objects and [[Def - Continuous Map|continuous maps]] as morphisms. A *category* consists of objects together with arrows between them, an associative composition, and identity arrows; here composition is ordinary function composition and the identity arrow $\mathrm{id}_X$ is the identity function. The four-axiom definition is, from this angle, just the data that makes "continuous map" well-defined and stable under composition. The category $\mathbf{Top}$ is rich: it has products (the [[Def - Product Topology|product topology]] is the categorical product), coproducts (disjoint union with the coproduct topology), equalizers and coequalizers (yielding the [[Def - Subspace Topology|subspace]] and quotient topologies as the natural ways of cutting and gluing spaces). Every standard construction in point-set topology is a universal-property construction in $\mathbf{Top}$.

**A topological space is a *frame*, equivalently a *locale* viewed pointlessly.** This is the more structural formulation, and it reveals that the open sets — not the points — carry the topological content. A *frame* is a complete lattice $L$ in which arbitrary joins distribute over finite meets:
$$U \wedge \bigvee_{\alpha \in I} V_\alpha = \bigvee_{\alpha \in I} (U \wedge V_\alpha).$$
For a topological space $(X, \tau)$, the collection $\tau$ of open sets is a frame under set inclusion: meet is intersection (finite intersections of opens are open, axiom 3), join is union (arbitrary unions of opens are open, axiom 2), top is $X$, bottom is $\emptyset$ (axiom 1), and the distributive law holds set-theoretically. The three topology axioms are thus equivalent to the single statement "$\tau$ is a frame, with operations inherited from $\mathcal{P}(X)$". A *locale* is the same thing as a frame but viewed in the opposite category — locales are to frames what spaces are to ring-of-functions: a morphism of locales $L \to L'$ is by definition a frame homomorphism $L' \to L$ going backwards, so that locale-morphisms compose covariantly like maps of spaces. The forgetful functor $\mathbf{Top} \to \mathbf{Loc}$, $(X, \tau) \mapsto \tau$, has a right adjoint $\mathbf{Loc} \to \mathbf{Top}$ sending a locale to its space of *points* (frame homomorphisms to $\{0, 1\}$), and the adjunction is an equivalence on the subcategory of *sober* spaces.

There is a third formulation worth naming. The lattice of open sets of any topological space is a **complete Heyting algebra** — a frame in which every element has a relative pseudo-complement $U \Rightarrow V := \bigvee\{W : W \wedge U \leq V\}$. This is exactly the lattice-theoretic interior of $V \cup (X \setminus U)$. The fact that open-set lattices are Heyting algebras (rather than Boolean algebras — the lattice fails complementation: the open set $U$ has no "open complement" in general) is the source of *intuitionistic* logic in topos theory: open sets model "propositions with constructive evidence", and the natural logic of a topological space is intuitionistic, not classical. The further generalisation — a *Grothendieck topology* on an arbitrary category, replacing the open-set lattice by a system of covering sieves — produces the right framework for sheaves on schemes, where the Zariski topology of [[Def - Ring|commutative rings]] is too coarse. The four axioms of a Grothendieck topology (stability under pullback, transitivity, and identity-covering) are the appropriate axioms once one drops the assumption that "open" is a property of a subset rather than of a morphism into the space.
