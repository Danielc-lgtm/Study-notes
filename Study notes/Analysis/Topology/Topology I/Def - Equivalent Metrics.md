...
**Is NOT an instance — the sup metric and the $L^1$ metric on $C[0, 1]$ are not topologically equivalent.** Consider $f_n(t) = $ "thin tall spike" of height $1$ and width $1/n$, supported near $t = 0$. Then $d_\infty(f_n, 0) = 1$ for all $n$ (the spike has height 1), but $d_1(f_n, 0) = \int |f_n| = 1/(2n) \to 0$. So $f_n \to 0$ in the $L^1$ metric but not in the sup metric. If the two metrics were topologically equivalent, convergence in one would imply convergence in the other — see [[Ex - Sup metric versus L1 metric on C[0,1]]]. This example is the prototype of "different metrics on a function space give genuinely different topologies", and it is the reason functional analysis needs to choose a metric carefully for each problem.
