...
The exercises of §1.2 drill the chain rule and its consequences. The matrix form $J(g \circ \Phi) = Jg(\Phi) \cdot J\Phi$ converts a coordinate change into a multiplicative Jacobian computation; the mean value inequality $|f(b) - f(a)| \leq \sup\|Df\| \cdot |b - a|$ controls a function via its derivative on a convex domain; and differentiating a function along a curve produces directional-derivative identities that encode rotational symmetry. The unifying technique: derivatives transfer between coordinate systems via the chain rule.
