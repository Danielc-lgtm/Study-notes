...
The exercises of §1.1 drill the regularity ladder for multivariate functions: continuous partials → differentiable → partials exist + continuous → partials exist. Each exercise calibrates one strict gap: a function with partials everywhere but not differentiable; a function differentiable but not continuously so; and the polar coordinate Jacobian as the prototype computation. The unifying observation: differentiability in several variables is much stronger than partial-derivative existence and is the right notion for the chain rule.
