...
The exercises of §3.2 drill Fubini's theorem and parameter-dependent integrals. Iterated integrals over non-rectangular regions reduce to nested one-variable problems; the order can be reversed when one direction admits an elementary antiderivative and the other does not (the standard $e^{y^2}$ trick); and differentiation under the integral sign with respect to a parameter converts a stubborn integral into an ODE — the Frullani identity $\int (e^{-ax} - e^{-bx})/x\,dx = \ln(b/a)$ as the canonical illustration.
