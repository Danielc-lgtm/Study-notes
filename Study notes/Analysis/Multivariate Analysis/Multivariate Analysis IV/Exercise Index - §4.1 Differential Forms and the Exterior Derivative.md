...
The exercises of §4.1 drill the algebra and calculus of differential forms. Wedge products, exterior derivatives, and pullbacks are each computed mechanically; the foundational identity $d \circ d = 0$ encodes both Schwarz's theorem on mixed partials and wedge antisymmetry in one stroke; and pullback computations confirm that the Jacobian determinant appears as $F^*(dx \wedge dy) = (\det DF)\,dr \wedge d\theta$. The unifying observation: differential forms package the algebra of multivariate calculus (gradient, curl, divergence) into a single coordinate-free framework with built-in antisymmetry.
