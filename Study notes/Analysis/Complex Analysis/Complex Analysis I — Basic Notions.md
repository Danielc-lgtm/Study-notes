...
5. **Topology — Logarithm and Winding Number.** The obstruction to defining $\log z$ on $\mathbb{C}^\times$ is exactly that $\mathbb{C}^\times$ is not simply connected — it has a loop around the origin with winding number $1$. The notion of winding number, defined in [[Topology IV — §13–17 Quotients, Homotopy, Topological Groups, Baire|Topology IV]] via homotopy classes of paths, is the topological obstruction. This is the first appearance of the connection that defines [[Topology IV — §13–17 Quotients, Homotopy, Topological Groups, Baire|the fundamental group]]: $\pi_1(\mathbb{C}^\times) = \mathbb{Z}$, and the integer is the winding number. The whole theory of branches and analytic continuation rests on this.
