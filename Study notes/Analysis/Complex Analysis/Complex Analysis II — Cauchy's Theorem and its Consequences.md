...
- **[[Ex - ML estimate bounds a circular integral]]** (⭐)
	- Use the [[Thm - ML Estimate|ML estimate]] to show $|\oint_{|z|=R} (z^2 + 1)/(z^4 + z + 1)\,dz| \to 0$ as $R \to \infty$. Bound the numerator from above by $R^2 + 1$ (forward triangle inequality) and the denominator from below by $\tfrac{1}{2}R^4$ (reverse triangle inequality, valid for $R \geq 2$), giving $|f| \leq 4/R^2$ on $|z| = R$; combined with $L = 2\pi R$, $|\text{integral}| \leq 8\pi/R \to 0$. Generalises to: any rational $f$ with $\deg(\text{denom}) \geq \deg(\text{num}) + 2$ has vanishing contour integral on large circles; the borderline residue case $1/z$ correctly fails the test.

- **[[Ex - A primitive exists iff integral around closed curves is zero]]** (⭐⭐)
	- Make the equivalence "primitive exists $\iff$ closed integrals vanish" concrete via $f(z) = 1/z$. Direct computation gives $\oint_{|z|=1} dz/z = 2\pi i \neq 0$, ruling out a primitive on $\mathbb{C}^\times$. On the slit plane $\mathbb{C} \setminus (-\infty, 0]$, exhibit the principal branch $\log z = \log|z| + i\arg z$ as an explicit primitive (verify $F'(z) = 1/z$ via Cauchy–Riemann), and use the [[Thm - Fundamental Theorem of Contour Integration|fundamental theorem]] to show every closed integral on the slit plane vanishes. The obstruction to a global primitive on $\mathbb{C}^\times$ is *topological*, not local — local primitives exist at every point.
...
- **[[Ex - Cauchy estimates bound the growth of an entire function]]** (⭐⭐)
	- Generalise Liouville from $n = 0$ to every $n \geq 0$: if $f$ is entire and $|f(z)| \leq A|z|^n$ for $|z| \geq R_0$, then $f$ is a polynomial of degree $\leq n$. Use [[Thm - Cauchy Estimates|Cauchy estimates]] at $0$ with $M(R) \leq A R^n$ to get $|c_k| \leq A R^{n - k}$ for every $R \geq R_0$. For $k > n$, $R^{n - k} \to 0$ as $R \to \infty$, so $c_k = 0$ and the Taylor series truncates. The signature growth-controls-structure pattern of §2.4.

- **[[Ex - Maximum modulus pins a function via boundary values]]** (⭐⭐)
	- $f$ holomorphic on $D$, continuous on $\bar D$, $|f| = 1$ on $\partial D$, $f(0) = 1$ — show $f \equiv 1$. Apply the [[Thm - (Global) Maximum Modulus Principle|global max modulus principle]] to get $|f| \leq 1$ on $\bar D$; the value $|f(0)| = 1$ makes the maximum attained at the interior point $0$, so by the *strong* form $f$ is constant on $D$, and $f(0) = 1$ identifies the constant. Alternative two-sided route via $1/f$ gives $|f| \geq 1$, hence $|f| \equiv 1$ and constancy. Canonical "interior maximum forces constancy" trigger.
