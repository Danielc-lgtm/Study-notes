...
A holomorphic function is precisely a **morphism in the category of complex manifolds** (or, at the local level, of open subsets of $\mathbb{C}$ with the standard complex structure). The categorical viewpoint pinpoints what makes complex differentiability special among notions of "smoothness".

The vocabulary: a *category* $\mathcal{C}$ has objects, arrows between objects, an associative composition, and identity arrows. The category $\mathbf{CplxMan}$ has *complex manifolds* — Hausdorff second-countable topological spaces with a maximal atlas of charts to open subsets of $\mathbb{C}^n$ whose transition maps are holomorphic — as objects, and holomorphic maps as arrows. Restricting to one complex dimension and to open subsets of $\mathbb{C}$ as objects gives the full subcategory whose arrows are exactly the holomorphic functions defined above. Composition is function composition (the composite of two holomorphic functions is holomorphic, by the chain rule), and identities are identity maps.

What distinguishes the holomorphic category from the smooth-manifold category $\mathbf{SmoothMan}$ — both viewing $\mathbb{C} \cong \mathbb{R}^2$ — is the *strengthening of morphisms*. A smooth map $f : U \to V$ between open subsets of $\mathbb{R}^2$ has a real-linear total derivative $Df_w : \mathbb{R}^2 \to \mathbb{R}^2$ at each point; a holomorphic map is a smooth map whose derivative is moreover **$\mathbb{C}$-linear**, i.e. commutes with the complex-structure operator $J : \mathbb{R}^2 \to \mathbb{R}^2$, $J(x, y) = (-y, x)$ (multiplication by $i$). The condition "$Df_w \circ J = J \circ Df_w$" is exactly the Cauchy–Riemann equations spelled out as a categorical demand: a holomorphic map is a smooth map whose derivative respects the additional complex-structure data, an *intertwiner* of complex structures.

There is a still more compressing categorical statement, due to the conformal characterisation: a $C^1$ map between open subsets of $\mathbb{C}$ is holomorphic *with non-vanishing derivative* if and only if it is **angle-preserving and orientation-preserving**. The category of complex domains can therefore be characterised as "the subcategory of real-differentiable maps whose differentials are everywhere conformal and orientation-preserving", with the holomorphic structure on $\mathbb{C}$ recovered as the unique structure making this work. This is the topology-and-geometry shadow of the algebraic statement that $\mathbb{C}$ is the unique $\mathbb{R}$-algebra of dimension 2 in which the squaring map is conformal. Complex analysis is the geometry of this category, and the rigidity theorems (Liouville, the [[Thm - Identity Theorem (Uniqueness of Analytic Continuation)|identity theorem]], the [[Thm - (Global) Maximum Modulus Principle|maximum modulus principle]]) are global obstructions characteristic of these especially restrictive arrows. By contrast, in the smooth category $C^\infty(\mathbb{R}^2, \mathbb{R}^2)$ has no such rigidity, which is why the orientation-and-angle-preserving cut is dramatic.
