...
*Example 5 (validation for $\lambda$).* In the missing-measurement variant, randomly remove some measurements, run the Kalman filter on the remaining ones for a range of $\lambda$, and compare the filter's *prediction* of the missing measurements against their actual values. This gives a held-out evaluation procedure for choosing $\lambda$, analogous to cross-validation in regression. Boyd §17.3.3 demonstrates this on the 2D tracking example.
