...
**Corollary ([[Def - Dimension|dimensions]]).** $\dim V^{(m)} = n^m$, where $n = \dim V$. For $m \geq 2$, this grows exponentially, which is why multilinear-form spaces become enormous quickly: even $V^{(3)}$ for $\dim V = 3$ already has dimension 27.
