...
- [[Ex - Best low-rank approximation via SVD]] (⭐⭐⭐) — Eckart–Young theorem: truncate the SVD to its top $k$ singular components for the best rank-$k$ approximation, with the lower bound proved by the **[[Def - Dimension|dimension]] intersection argument** (kernel of $B$ intersects top-$(k+1)$ right-singular span in a non-trivial vector) ([[Thm - Singular Value Decomposition]], [[Def - Singular Values]], [[Ex - SVD computes the operator norm]])
