...
For (6), the null-space-range duality: $w \in \operatorname{null} T^*$ iff $T^* w = 0$ iff $\langle v, T^* w \rangle = 0$ for all $v$ iff $\langle Tv, w \rangle = 0$ for all $v$ iff $w \in (\operatorname{range} T)^\perp$. The other three relations follow by replacing $T$ with $T^*$ and applying $T^{**} = T$ and the involution of orthogonal complement $U^{\perp\perp} = U$ (for closed [[Def - Subspace|subspaces]] in finite [[Def - Dimension|dimensions]], all [[Def - Subspace|subspaces]] are closed).
