...
Let $U(2) = \{T \in \operatorname{GL}_2(\mathbb{C}) : T^* T = I\}$, the unitary [[Def - Group|group]] of $\mathbb{C}^2$.
...
**Key decision point.** Using the structure that an [[Def - Isometry|isometry]] $\mathbb{C}^2 \to \mathbb{C}^2$ is determined by where the basis vectors go (up to orthogonality). The first basis vector determines a unit vector in $\mathbb{C}^2$; the second is then constrained to a unit vector orthogonal to the first, giving the form in (b).
...
> [!note]- Hint 1
> [[Def - Group|Group]] axioms: closure ($U_1 U_2$ unitary if $U_1, U_2$ are), inverses ($U^{-1} = U^*$), identity ($I$). Each is direct.
...
*Inverses.* If $U \in U(2)$, then $U^{-1} = U^*$ (since $U^* U = I$). Is $U^* \in U(2)$? Check: $(U^*)^* (U^*) = U U^* = I$ (using $UU^* = I$, which follows from $U^*U = I$ in finite [[Def - Dimension|dimensions]]). So $U^{-1} = U^* \in U(2)$.

*Associativity.* Matrix multiplication is associative.

> [!note]- Derivation
> The check for inverses uses the finite-dimensional fact that $U^*U = I$ implies $UU^* = I$. In infinite [[Def - Dimension|dimensions]] this need not hold (the unilateral shift is an [[Def - Isometry|isometry]] but not a unitary).
