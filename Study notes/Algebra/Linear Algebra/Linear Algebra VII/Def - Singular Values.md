...
For a projection matrix $P_U$ (orthogonal projection onto $U$): $P_U^* P_U = P_U^2 = P_U$, eigenvalues $\{1, \ldots, 1, 0, \ldots, 0\}$ ($\dim U$ ones and $\dim U^\perp$ zeros), so singular values are $\{1, \ldots, 1, 0, \ldots, 0\}$. The unit ball is mapped to the unit ball in $U$ (a disk of [[Def - Dimension|dimension]] $\dim U$), which has $\dim U$ semi-axes of length $1$ and $\dim U^\perp$ semi-axes of length $0$.
