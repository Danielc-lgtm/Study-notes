...
Why is the isometry a useful class of maps? Three reasons. First, the **composition of isometries is an isometry**, so the set of isometries on a fixed space $V$ forms a monoid (and, in finite dimensions, a group — the orthogonal or unitary group). Second, isometries are exactly the **injective** norm-preserving linear maps — norm preservation forces injectivity, because $Sv = 0$ implies $\|v\| = 0$ implies $v = 0$. Third, an isometry can be inverted on its image (it is a linear isomorphism onto its range), so it provides an isomorphism of $V$ with a subspace of $W$.
