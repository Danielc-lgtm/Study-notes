...
A **morphism** of inner product spaces — an **isometry** — is a linear map $T : V \to W$ preserving the inner product, $\langle Tu, Tv\rangle_W = \langle u, v\rangle_V$. Isometries are the natural notion of "equivalence of geometry" on vector spaces, and they form a group $O(V)$ (the orthogonal group, when $V$ is real) or $U(V)$ (the unitary group, when $V$ is complex).
