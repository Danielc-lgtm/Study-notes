...
The dual identity is **$A^T v$ is the edge potential drop**. The transpose $A^T$ has [[Def - Dimension|dimensions]] $m \times n$; given a node potential vector $v$ (one number per node), its product $A^T v$ has one number per edge. Working through the algebra: $(A^T v)_j = \sum_i A_{ij} v_i$, and only the two nonzero rows contribute: $(A^T v)_j = v_{\operatorname{head}(j)} - v_{\operatorname{tail}(j)}$, the potential at the head minus at the tail. If $v$ represents an electrical potential or temperature, $A^T v$ is the voltage drop across each edge.
