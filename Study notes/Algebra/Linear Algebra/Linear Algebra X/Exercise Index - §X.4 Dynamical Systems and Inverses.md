...
- [[Ex - Pseudoinverse for least squares (Boyd)]] (⭐⭐) — show that for a tall full-rank $A$, the pseudoinverse $A^\dagger = (A^T A)^{-1} A^T$ is a left inverse and that $\hat x = A^\dagger b$ minimises $\|Ax - b\|^2$, via the orthogonality condition $A^T(b - A\hat x) = 0$ ([[Def - Left and Right Inverse of a Matrix]], [[Def - Norm and Distance]])
