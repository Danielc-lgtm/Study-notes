...
The factor theorem itself has a clean categorical reading: it says the evaluation ring homomorphism $\operatorname{ev}_\lambda : F[x] \to F$, $p \mapsto p(\lambda)$, has kernel exactly $(x - \lambda)$. By the [[Thm - First Isomorphism Theorem for Rings|first isomorphism theorem]], $F[x]/(x - \lambda) \cong F$, which is a field — so the ideal $(x - \lambda)$ is **maximal** in $F[x]$. In fact every maximal ideal of $F[x]$ has the form $(p)$ for $p$ a monic irreducible polynomial, by the PID structure, and the [[Def - Residue|residue]] field $F[x]/(p)$ is a field extension of $F$ of degree $\deg p$. So the factorization theorems over $\mathbb{C}$ and $\mathbb{R}$ are statements about the **prime [[Def - Ideal|ideals]]** of the polynomial [[Def - Ring|rings]] $\mathbb{C}[x]$ and $\mathbb{R}[x]$: $\mathbb{C}[x]$ has prime [[Def - Ideal|ideals]] only of the form $(z - \lambda)$, all maximal of [[Def - Residue|residue]] field $\mathbb{C}$, whereas $\mathbb{R}[x]$ also has primes of the form $(x^2 + bx + c)$ with $b^2 < 4c$, maximal of residue field $\mathbb{C}$.
