...
Specifically, the null spaces $\operatorname{null} T^k$ form an increasing sequence, and once two consecutive terms coincide, all later terms coincide. The chain must stabilise by index $\dim V$ at the latest, because a strictly increasing chain of [[Def - Subspace|subspaces]] in $V$ adds at least one dimension at each step, and $V$ has only $\dim V$ [[Def - Dimension|dimensions]].
