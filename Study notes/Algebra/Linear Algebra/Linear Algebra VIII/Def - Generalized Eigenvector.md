...
What is the payoff of the definition? On a complex space, the generalized eigenvectors of $T$ for each eigenvalue $\lambda$ (together with zero) form a *subspace* $G(\lambda, T)$, called the **generalized eigenspace**, and these subspaces *always* sum to all of $V$:
