...
**Polynomial linear independence is "coefficients are uniquely determined".** The exercise turns on the fact that two polynomial expressions giving the same function on an infinite field must have the same coefficients. This is *the* polynomial-[[Def - Ring|ring]] fact that makes linear-algebraic arguments about polynomial spaces work. The list $1, z, \ldots, z^n$ is linearly independent precisely because no two distinct polynomials of degree $\leq n$ give the same function — i.e. the coefficient map is injective. Whenever you need to prove a list of polynomials linearly independent, the fundamental tool is: write out a vanishing combination, treat it as a polynomial in $z$, and read off coefficients (either by polynomial-degree arguments, by evaluating at well-chosen points, or by repeated differentiation).

**The off-by-one: degree at most $n$, basis of length $n + 1$.** This is the most common point of confusion in polynomial-space [[Def - Dimension|dimension]]. $\mathcal{P}_n(F)$ — degree at most $n$ — has basis $1, z, z^2, \ldots, z^n$ of length $n + 1$, so $\dim \mathcal{P}_n(F) = n + 1$. The list includes the constant polynomial $1 = z^0$ as the first entry. Always count basis vectors, not maximum exponent.
