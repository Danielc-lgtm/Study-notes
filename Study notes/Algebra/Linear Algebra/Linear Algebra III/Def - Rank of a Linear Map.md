...
The rank of a linear map is the answer to the most basic question: **how big is the actual output?** A linear map $T : V \to W$ has its output set $\operatorname{range} T \subseteq W$, and this is a [[Def - Subspace|subspace]]; its dimension is the rank. Rank is the cleanest single-number summary of "what $T$ does" — it measures the *effective dimension* of the operation, ignoring whatever happens inside the null space.

Why a *single number* and not the whole range subspace? Two reasons. First, the dimension is what one can manipulate algebraically: [[Def - Dimension|dimensions]] add, the inequality $\operatorname{rank}(ST) \leq \min\{\operatorname{rank} S, \operatorname{rank} T\}$ becomes a numerical bound, and conservation laws like [[Thm - Fundamental Theorem of Linear Maps|rank–nullity]] connect ranks to other [[Def - Dimension|dimensions]]. Second, the rank is a *basis-free invariant* of $T$: it does not depend on any choice of basis in $V$ or $W$. The range itself is a subspace of $W$, and its identity-as-a-set depends on $W$'s structure; but its dimension is a single integer attached to $T$ alone.
