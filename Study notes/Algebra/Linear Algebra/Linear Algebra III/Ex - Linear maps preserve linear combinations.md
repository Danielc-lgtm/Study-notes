...
**Linearity is preserved by induction on linear combinations.** The two axioms — additivity and homogeneity — combine to produce the *true name* of linearity: $T(\sum \lambda_k v_k) = \sum \lambda_k T v_k$. The reusable principle is that two algebraic axioms, each handling one operation, combine via induction to handle the operation iterated arbitrarily many times. This is the same pattern that converts the group axiom "the product of two elements is in the group" into "the product of any finite list of elements is in the group", and similar combinations across algebra. The trigger is "axioms for two operations, want to handle finite combinations". The induction-on-length argument is the standard tool.
