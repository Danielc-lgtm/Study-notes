...
**Non-example: a non-square "change of basis".** A change-of-basis matrix is always square ($n$-by-$n$, where $n = \dim V$). One does not "change basis" between spaces of different dimensions — that would be a change between non-isomorphic vector spaces, which is meaningless.
