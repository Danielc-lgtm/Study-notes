...
**[[Def - Dimension|Dimension]] counting gives $\dim \mathcal{L}(\mathbb{R}, \mathbb{R}) = 1$.** The result of this exercise is that $\mathcal{L}(\mathbb{R}, \mathbb{R}) \cong \mathbb{R}$ — every linear map is determined by a single scalar. [[Def - Dimension|Dimension]]-counting: $\dim \mathcal{L}(V, W) = (\dim V)(\dim W) = 1 \cdot 1 = 1$, by [[Ex - The space of linear maps has dimension mn|the $mn$ theorem]]. This is the smallest non-trivial confirmation of the dimension formula, and it generalises: $\dim \mathcal{L}(\mathbb{R}^n, \mathbb{R}^m) = mn$, so the space of $n$-input-$m$-output linear maps is itself an $mn$-dimensional space. Every map is "$mn$ scalars", arranged as a matrix.
