...
**Key decision point.** The crucial recognition is the "socks-and-shoes" reversal: putting on socks then shoes is undone by removing shoes then socks. The reversal of order is mandatory; one cannot avoid it. The "key decision" is to verify both compositions, since invertibility requires *both* $R(TS) = I$ and $(TS) R = I$. (Each implies the other in finite dimensions by [[Thm - Injectivity Equals Surjectivity in Finite Dimensions]], but in general we verify both.)
