...
**Is an instance: differentiation $D : \mathcal{P}(\mathbb{R}) \to \mathcal{P}(\mathbb{R})$, $Dp = p'$.** Additivity is $(p + q)' = p' + q'$ and homogeneity is $(\lambda p)' = \lambda p'$ — both are basic facts of calculus. This is the canonical example of a linear map on an infinite-dimensional space, and its null space (constants) and range (everything) illustrate how kernel and image behave outside finite [[Def - Dimension|dimensions]].
