...
**Both are subspaces.** $\operatorname{null} T$ is a subspace of $V$, and $\operatorname{range} T$ is a subspace of $W$. (Each contains the zero vector of the respective space, and each is closed under addition and scalar multiplication; the verifications are one line apiece.)
