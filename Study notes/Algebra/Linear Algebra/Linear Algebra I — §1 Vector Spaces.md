...
The reader is assumed to have refreshed: basic [[Def - Group|group theory]] (especially [[Def - Abelian Group|abelian groups]]; the additive structure of a vector space is one), enough [[Def - Ring|ring theory]] to know what a [[Def - Field|field]] is (in particular $\mathbb{Q}$, $\mathbb{R}$, $\mathbb{C}$, $\mathbb{F}_p$), and elementary set theory (functions, intersections, unions, lattice of subsets). No analysis is required for this topic; the function-space examples are used algebraically, not analytically.
...
> [!tip] Unlocked: Vector Space Itself *(from Linear Algebra)*
...
> [!tip] Unlocked: Algebraic Closure and the Fundamental Theorem of Algebra *(from Algebra)*
...
> [!tip] Unlocked: Module *(from Algebra)*
> Replacing the field $\mathbb{F}$ by an arbitrary [[Def - Ring|ring]] $R$ gives a [[Def - Module|module]] over $R$, a strictly more general object whose theory loses several of linear algebra's clean theorems — bases need not exist, dimension is not always well-defined, and not every submodule is a direct summand. This is the systematic generalization developed in [[Def - Module|module theory]]. The features that distinguish vector spaces from modules (every space has a basis, every short exact sequence splits, every subspace is a direct summand) all come from the field axiom of multiplicative inverses for nonzero elements.
...
> [!tip] Unlocked: Tangent Space *(from Differential Geometry)*
...
> [!tip] Unlocked: Quotient Space *(from Linear Algebra IV)*
...
> [!tip] Unlocked: Span and Linear Independence *(from Linear Algebra II)*
...
> [!tip] Unlocked: Orthogonal Complement and Spectral Decomposition *(from Linear Algebra VI–VII)*
