...
When $V_1, \dots, V_m$ are *subspaces* of a common ambient $V$, there is also a natural linear map
...
**Is an instance — $\mathbb{F}^n$ as $\mathbb{F} \times \cdots \times \mathbb{F}$.** The coordinate vector space $\mathbb{F}^n$ is the $n$-fold product of the field $\mathbb{F}$ with itself. Addition is componentwise, scalar multiplication is componentwise, the projections are coordinate projections $\pi_k(x_1, \dots, x_n) = x_k$, and $\dim \mathbb{F}^n = n$ by the dimension theorem below. This is the *first* example one should hold in mind when meeting the product definition: $\mathbb{F}^n$ has been the product all along.
