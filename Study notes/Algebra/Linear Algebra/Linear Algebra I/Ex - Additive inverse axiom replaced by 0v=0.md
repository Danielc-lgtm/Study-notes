...
**The corollary $0 \cdot v = 0$ is doing real work, not just bookkeeping.** In the standard formulation, $0 \cdot v = 0$ is a *theorem* ([[Thm - Uniqueness of Additive Identity and Inverses]]) derived from the axioms; in this exercise's reformulation it is an *axiom* generating additive inverses. The fact that the same identity can play either role — theorem or axiom — depending on the formulation reveals that it encodes nontrivial structural content. In other algebraic settings (rings, modules, algebras over a field), the analogous "zero-scalar kills" identity is similarly load-bearing.
