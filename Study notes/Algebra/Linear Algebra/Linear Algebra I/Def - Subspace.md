...
$V$ denotes a vector space over a field $\mathbb{F}$. A subspace is typically denoted $U, W$, or $V_1, V_2, \dots$. The notation $U \leq V$ is not standard in linear algebra; instead one writes "$U$ is a subspace of $V$" or simply $U \subseteq V$ with the qualifier "subspace" understood. The zero subspace is $\{0\}$, often just written $0$. See [[Linear Algebra I — §1 Vector Spaces]] for the full notation registry.
...
**Is an instance: $\mathcal{P}_m(\mathbb{F}) \subset \mathcal{P}(\mathbb{F})$.** The polynomials of degree at most $m$ form a finite-dimensional subspace ([[Def - Dimension|dimension]] $m + 1$, with basis $1, x, \dots, x^m$) of the infinite-dimensional space of all polynomials. The zero polynomial has degree $-\infty < m$, the sum of two polynomials of degree $\leq m$ has degree $\leq m$, and any scalar multiple stays in the same degree class.
