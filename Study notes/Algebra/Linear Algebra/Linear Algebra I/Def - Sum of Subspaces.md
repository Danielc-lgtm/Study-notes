...
The thing we are trying to axiomatize is **the smallest subspace containing several given [[Def - Subspace|subspaces]]**. The natural set-theoretic operation here would be the union $V_1 \cup \dots \cup V_m$ — for an ordinary set, the union is the smallest set containing the pieces. But the union of [[Def - Subspace|subspaces]] fails to be a subspace in almost every interesting case. Take $V_1$ = $x$-axis and $V_2$ = $y$-axis in $\mathbb{R}^2$: each is a subspace, $V_1 \cup V_2$ is the union of the two axes, but $(1, 0) + (0, 1) = (1, 1)$ is not in either axis. So we cannot use the union as the join in the lattice of subspaces; we need something larger.
