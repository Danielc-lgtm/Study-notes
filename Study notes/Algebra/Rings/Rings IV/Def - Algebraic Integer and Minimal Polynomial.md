...
Start from a concrete wish. The integers $\mathbb{Z}$ are the basic objects of arithmetic, but they are not closed under the operations we care about: solving polynomial equations. The number $i$ is not an integer, yet it satisfies $i^2 + 1 = 0$ — an equation as clean and integral as anything in $\mathbb{Z}$. The number $\sqrt{2}$ satisfies $X^2 - 2 = 0$. We would like a notion of "integer" wide enough to admit these — a class of complex numbers that behaves like $\mathbb{Z}$, supports a sensible factorisation theory, and forms a ring — without simply admitting *all* of $\mathbb{C}$, which has no arithmetic of divisibility at all.
