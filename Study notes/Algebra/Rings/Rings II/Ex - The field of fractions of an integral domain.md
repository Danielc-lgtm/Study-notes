...
2. The **polynomial ring** $F[X]$ over a field $F$. Show $\operatorname{Frac}(F[X])=F(X)$, the field of **rational functions** in $X$ — formal quotients $p(X)/q(X)$ of polynomials with $q\neq 0$.
...
**Assumption pattern.** In both parts the domain $R$ already sits inside a field: $\mathbb{Z}[i]$ inside $\mathbb{C}$, and $F[X]$ inside the obvious field of rational functions. The recognisable pattern is: *the domain $R$ is given as a subring of a known field $K$, and one suspects $K$ is "just big enough"*. When that is the situation, you never construct anything — you cut $K$ down to the smallest subfield containing $R$ and check it equals the candidate.
