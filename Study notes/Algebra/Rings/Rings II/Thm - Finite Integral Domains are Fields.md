...
**Endomorphism rings and Schur's lemma.** In representation theory, Schur's lemma says the endomorphism ring of a finite simple module over a finite ring is a division ring; when it is commutative it is a domain, and if the module is finite the endomorphism ring is a finite set. The theorem then makes that endomorphism ring a field. This is out-of-distribution because the ring in question is a ring of *maps*, not of numbers, yet finiteness of the underlying module forces the endomorphism ring to be finite, and the domain property (no nonzero endomorphism kills the simple module) triggers the theorem.

**Quotients of $\mathbb{Z}[i]$ at a Gaussian prime.** For a Gaussian prime $\pi \in \mathbb{Z}[i]$, the quotient $\mathbb{Z}[i]/(\pi)$ is finite (the norm $N(\pi)$ bounds its size) and is a domain because $(\pi)$ is a prime ideal. The theorem yields that $\mathbb{Z}[i]/(\pi)$ is a *field*, which is how one shows Gaussian primes give residue fields $\mathbb{F}_p$ or $\mathbb{F}_{p^2}$. The non-obvious step is that a quotient of an infinite ring lands in the theorem's hypothesis purely because the relevant ideal has finite index.
