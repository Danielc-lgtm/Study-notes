...
**Corollary — a ring embeds in a field if and only if it is an integral domain.** One direction is the existence theorem ([[Thm - Existence of the Field of Fractions]]); the other is the easy observation that any [[Def - Subring|subring]] of a field is an integral domain (a field has no zero divisors, and the property is inherited by subrings). Together they give a complete, memorable characterisation: *embeddability in a field $\iff$ no zero divisors*.
