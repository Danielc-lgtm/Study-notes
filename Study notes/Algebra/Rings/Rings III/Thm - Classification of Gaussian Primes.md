...
>
> > [!note]- Full proof
> > Since $p \equiv 1 \pmod 4$, write $p - 1 = 4k$ for some integer $k \geq 1$. By [[Thm - The Unit Group of a Finite Field is Cyclic|the theorem that $\mathbb{F}_p^\times$ is cyclic]], the group $\mathbb{F}_p^\times = (\mathbb{Z}/p\mathbb{Z}) \setminus \{0\}$ is cyclic of order $p - 1 = 4k$. A cyclic group of order $n$ has an element of order $d$ for every divisor $d$ of $n$; since $4 \mid 4k$, there is an element $[a] \in \mathbb{F}_p^\times$ of order exactly $4$.
> >
...
- **[[Thm - The Unit Group of a Finite Field is Cyclic|Cyclicity of $\mathbb{F}_p^\times$]]** — the essential lemma for the hard direction. Without it there is no guarantee that $-1$ has a square root modulo $p$ when $p \equiv 1 \pmod 4$, and the proof that such $p$ split would collapse. The classification is, in a precise sense, the cyclicity theorem cashed out in $\mathbb{Z}[i]$.
