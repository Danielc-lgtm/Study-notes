...
There is nothing to *invent* in the definition of $\mathbb{Z}[i]$ itself — it is simply the smallest subring of $\mathbb{C}$ containing $i$, namely all $\mathbb{Z}$-linear combinations of $1$ and $i$. The genuine design question is the **norm**: why attach the function $N(a+bi) = a^2 + b^2$ to this ring, and why is it *this* function and not a nearby variant? Understanding that is understanding why the Gaussian integers are tractable at all.
