...
The [[Def - Gaussian Integers|Gaussian integers]] $\mathbb{Z}[i]=\{a+bi:a,b\in\mathbb{Z}\}$ are a [[Def - Subring|subring]] of $\mathbb{C}$, with **norm** $N(a+bi)=a^2+b^2=(a+bi)(a-bi)$. The norm is **multiplicative**, $N(zw)=N(z)N(w)$, and its **units** are exactly the four elements of norm $1$:
