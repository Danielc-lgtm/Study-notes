...
> **Second Isomorphism Theorem for Rings.** Let $S$ be a ring, $R \leq S$ a subring, and $J \trianglelefteq S$ an ideal. Then:
...
1. **Build the map.** Define $\varphi : R \to S/J$ by $\varphi(r) = r + J$.
   - *Hint:* This is the composite $R \hookrightarrow S \twoheadrightarrow S/J$ — the inclusion followed by the canonical quotient map. A composite of ring homomorphisms is a ring homomorphism.
