...
2. Determine exactly which elements are **zero divisors**. Show that the zero divisors are precisely the nonzero [[Def - Residue|residues]] $\overline{a}$ with $\gcd(a, n) > 1$ — so every nonzero element of $\mathbb{Z}/n\mathbb{Z}$ is either a unit or a zero divisor, and never both.
...
A [[Def - Ring|ring]] $R$ is a set with two operations $+$ and $\cdot$: it is an abelian [[Def - Group|group]] under $+$, the multiplication is associative with an identity $1_R$, and multiplication distributes over addition. In this course every ring is commutative. The ring $\mathbb{Z}/n\mathbb{Z}$ has underlying set the $n$ residue classes modulo $n$; the class of an integer $a$ is written $\overline{a}$, and $\overline{a} = \overline{b}$ exactly when $n \mid (a - b)$.
...
> Fix $n \ge 2$.
>
> **Units.** $\overline{a}$ is a unit of $\mathbb{Z}/n\mathbb{Z}$ if and only if there is $\overline{x}$ with $\overline{a}\,\overline{x} = \overline{1}$, i.e. $\overline{ax} = \overline{1}$, i.e. $n \mid (ax - 1)$, i.e. there are integers $x, y$ with $ax + yn = 1$. By Bézout's identity the linear form $ax + yn$ takes the value $1$ if and only if $\gcd(a, n) = 1$. Therefore
...
>
> **Field criterion.** $\mathbb{Z}/n\mathbb{Z}$ is a nonzero ring, so it is a field if and only if every nonzero residue is a unit, if and only if no nonzero residue has $\gcd(a, n) > 1$, if and only if $n$ has no divisor $d$ with $1 < d < n$, if and only if $n$ is prime. $\blacksquare$
