...
> **First Isomorphism Theorem for Rings.** Let $\varphi : R \to S$ be a ring homomorphism. Then $\ker\varphi$ is an ideal of $R$, and the map
...
Here is the question it answers. You are handed a quotient ring $R/I$ and asked what it *is* — which familiar ring it equals up to isomorphism. Working with $R/I$ directly is unpleasant, because its elements are cosets, which are sets, and reasoning about arithmetic on sets is awkward. You would much rather recognise $R/I$ as a ring you already know. The theorem tells you precisely when you may: $R/I \cong T$ exactly when there is a *surjective* ring homomorphism $R \to T$ whose kernel is $I$. So the strategy flips. Instead of dissecting the quotient, you go hunting for a homomorphism *out of $R$* — and homomorphisms out of polynomial rings, in particular, are cheap to build, because a homomorphism out of $R[X]$ is determined by where it sends $X$.
