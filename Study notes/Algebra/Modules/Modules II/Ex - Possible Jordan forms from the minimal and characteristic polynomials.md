...
The objects in play are an endomorphism $\alpha$ of a complex vector space $V$, its associated $\mathbb{C}[X]$-module $V_\alpha$, and the Jordan normal form.
...
> - The exponent of $(X-\lambda)$ in $m_\alpha$ is $a_\lambda$, the *largest* $\lambda$-block size. (The polynomial $(X-\lambda)^k$ kills the summand $\mathbb{C}[X]/((X-\lambda)^m)$ iff $k \ge m$; to kill *all* $\lambda$-blocks one needs $k \ge \max m$, and the minimal such $k$ is the largest block size.)
>
> Hence:
