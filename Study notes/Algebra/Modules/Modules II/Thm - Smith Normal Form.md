...
> We argue by induction on $\min(m,n)$, the smaller [[Def - Dimension|dimension]] of the matrix; throughout we keep calling the matrix $A$ even though it changes at each step, to avoid inventing a name for every intermediate matrix. By Lemma 1 every operation is reversible, so each step keeps the matrix equivalent to the original $A$.
>
