...
1. Form the matrix $XI - A$ over the polynomial [[Def - Ring|ring]] $\mathbb{Q}[X]$ and reduce it to **Smith normal form** by elementary row and column operations over $\mathbb{Q}[X]$.
...
The point of the exercise is to see the structure theorem turned into an *algorithm*: the entire similarity classification of a matrix is extracted by Gaussian elimination over a polynomial [[Def - Ring|ring]].

**Recall:**

The objects in play are a field $F$, a matrix $A \in M_{n,n}(F)$, the polynomial ring $F[X]$, and the $F[X]$-[[Def - Module|module]] $V_A$ that $A$ defines.

![[Def - The Module of a Linear Operator#The Definition]]

Concretely: $V_A$ is the vector space $F^n$, on which the polynomial ring $F[X]$ acts by letting $X$ act as the matrix $A$ — so a polynomial $f(X)$ acts as the matrix $f(A)$. Because $F[X]$ is a [[Def - Euclidean Domain|Euclidean domain]] (it has the degree function as a Euclidean function, so it admits division with remainder), every finitely generated $F[X]$-[[Def - Module|module]] is classified by the [[Thm - Structure Theorem for Finitely Generated Modules over a Euclidean Domain|structure theorem]].
