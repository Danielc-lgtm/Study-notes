...
be a **monic** polynomial of degree $r$. The quotient ring $F[X]/(f)$ is, viewed as an $F$-vector space, $r$-dimensional with basis the residue classes of
$$1,\ X,\ X^2,\ \dots,\ X^{r-1}.$$
...
**Corollary (the action factors through a quotient ring).** If $\dim_F V = n < \infty$, then $V_\alpha$ is finitely generated and, since $F[X]$ is a [[Def - Euclidean Domain|Euclidean domain]] hence a principal ideal domain, the annihilator of $V_\alpha$ is a principal ideal $(m)$ for some monic $m$ — the **minimal polynomial** of $\alpha$. The $F[X]$-action then factors through the finite-dimensional quotient ring $F[X]/(m)$. *Calibration check:* recognising that finite dimension of $V$ forces a nonzero annihilator (the powers $\operatorname{id}, \alpha, \dots, \alpha^{n^2}$ cannot be $F$-linearly independent in the $n^2$-dimensional space $\operatorname{End}_F(V)$) confirms you see why $V_\alpha$ has no free part.
