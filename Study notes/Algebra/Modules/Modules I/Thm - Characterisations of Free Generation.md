...
Finally, the *uniqueness* of the extension (mentioned in the definition) is itself forced and worth seeing: if $\theta_1$ and $\theta_2$ both extend $\psi$, then $\theta_1 - \theta_2$ is a homomorphism killing every element of $S$, so it kills the entire submodule generated by $S$ — which is $M$. Hence $\theta_1 = \theta_2$. Generation alone forces uniqueness; freeness is the extra promise that *some* extension exists.
