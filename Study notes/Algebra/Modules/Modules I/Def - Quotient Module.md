...
**Is an instance: quotient vector space $V/W$.** If $F$ is a field, $V$ a vector space and $W$ a [[Def - Subspace|subspace]], the quotient module $V/W$ is the quotient vector space. For instance with $V = \mathbb{R}^2$ and $W$ the $x$-axis, $V/W$ is a $1$-dimensional space whose elements are the horizontal lines $\{(x, y_0) : x \in \mathbb{R}\}$; addition of lines and scaling of lines are induced from $\mathbb{R}^2$, and $\dim(V/W) = 2 - 1 = 1$.
