...
**Is an instance — principal fractional ideals.** In any domain, $(x) = xA$ for $x \in K^\times$ is a fractional ideal: take $d$ to be the denominator of $x$. These are exactly the elements of $\mathcal{P}(A)$, and they form the trivial class in $\operatorname{Cl}(A)$. In $\mathbb{Z}$ with $K = \mathbb{Q}$, the fractional ideal $\tfrac{2}{3}\mathbb{Z} = \{\tfrac{2n}{3} : n \in \mathbb{Z}\}$ is principal, generated by $\tfrac23$.
