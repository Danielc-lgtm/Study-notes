...
>
> *Example.* $f = Y^2 - X^3 - X$: monic in $Y$, so $u = x$, $y^2 = x^3 + x$, $A = k[x] \oplus k[x]y$ free of rank $2$, normalization $= $ projection $C \to \mathbb{A}^1$, $(x, y) \mapsto x$, a $2$-sheeted cover. $\blacksquare$
