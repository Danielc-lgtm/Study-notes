...
**Is an instance: the symmetric group $S_n$ itself.** Trivially $S_n \leq S_n$, so $S_n$ is a permutation group of degree $n$ — the maximal one, since it contains *all* bijections. Likewise $A_n \leq S_n$ is a permutation group of degree $n$. Every [[Def - Symmetric Group|symmetric group]] and every [[Def - Symmetric Group|alternating group]] is a permutation group; the notion of permutation group is the closure of "symmetric group" under passing to subgroups.
