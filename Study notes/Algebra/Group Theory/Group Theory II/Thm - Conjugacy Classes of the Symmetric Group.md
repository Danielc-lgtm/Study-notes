...
The payoff is concrete and immediate. Because we can list the classes and compute their sizes from the formula $n!/\prod_k k^{a_k}a_k!$, the class equation of $S_n$ becomes a genuine numerical object. This is the combinatorial input to the hardest result of the topic: the [[Thm - Simplicity of the Alternating Group|simplicity of $A_n$]] is proved by tracking which cycle types lie in a putative normal subgroup, and the brute-force verification that $A_5$ is simple is nothing but reading the $S_5$ class sizes, restricting to $A_5$, and checking that no sub-collection forms a normal subgroup. Cycle type is what makes that argument possible.
