...
[[Def - Conjugacy Class|Conjugacy]] is the right notion of "two elements are structurally the same", but for a general [[Def - Group|group]] it is invisible: deciding whether $x$ and $y$ are conjugate means searching all of $G$ for some $g$ with $gxg^{-1} = y$, an existence question with no shortcut. Conjugacy classes are defined, not computed. The [[Thm - The Class Equation|class equation]] can be written down only once you already know the classes, and for most [[Def - Group|groups]] that knowledge is hard-won.
