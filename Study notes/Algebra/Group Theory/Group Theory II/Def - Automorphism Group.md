...
The automorphism group of this page is this construction in the category $\mathbf{Grp}$ of groups and group homomorphisms: $\operatorname{Aut}(G) = \operatorname{Aut}_{\mathbf{Grp}}(G)$. The same construction in the category $\mathbf{Set}$ gives the [[Def - Symmetric Group|symmetric group]] $\operatorname{Sym}(X)$ — automorphisms of a bare set are just bijections. In $\mathbf{Vect}$ it gives the general linear group $\mathrm{GL}(V)$. In $\mathbf{Top}$ it gives the self-homeomorphisms of a space. So "automorphism group" is one categorical idea, and $\operatorname{Aut}(G) \leq \operatorname{Sym}(G)$ is the statement that a group-automorphism is in particular a set-automorphism — the forgetful functor $\mathbf{Grp} \to \mathbf{Set}$ induces an inclusion of automorphism groups.
