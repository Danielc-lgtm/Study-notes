...
**Corollary — $g$ is central iff its conjugacy class is a singleton.** The element $g$ lies in $Z(G)$ exactly when $\operatorname{ccl}_G(g) = \{g\}$, equivalently (by orbit-stabiliser) when $C_G(g) = G$. This is the bridge to the [[Thm - The Class Equation|class equation]]: the singleton classes are precisely the central elements, so the class equation isolates $|Z(G)|$ as the sum of the size-$1$ classes.

**Corollary — $G$ abelian iff $Z(G) = G$ iff every centraliser is all of $G$.** A group is [[Def - Abelian Group|abelian]] exactly when every pair commutes, i.e. every element commutes with every element, i.e. $C_G(g) = G$ for all $g$, i.e. $\bigcap_g C_G(g) = Z(G) = G$. The three conditions are restatements of one another, and verifying their equivalence is the calibration check for understanding both definitions.
