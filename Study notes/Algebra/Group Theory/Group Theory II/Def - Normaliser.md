...
Let $G$ be a group and $H \leq G$ a [[Def - Subgroup|subgroup]]. The **normaliser** of $H$ in $G$ is
...
**Is NOT an instance — the conjugate $gHg^{-1}$ is not "$H$ shifted".** A common slip is to picture $gHg^{-1}$ as a coset or a translate of $H$. It is neither: $gHg^{-1}$ is a *subgroup* (it contains $e$ and is closed), generally a different subgroup of the same order, whereas a coset $gH$ is *not* a subgroup unless $g \in H$. The normaliser is the set of $g$ for which the *subgroup* $gHg^{-1}$ coincides with $H$ — a statement about subgroups, not about cosets.
...
**Corollary — $H \trianglelefteq G$ iff $N_G(H) = G$ iff $H$ has exactly one conjugate.** Three equivalent statements of normality, each a calibration check. $H$ is normal precisely when every $g$ normalises it, precisely when the normaliser is everything, precisely when the conjugacy class of subgroups $\{gHg^{-1}\}$ is the singleton $\{H\}$ — i.e. $|G : N_G(H)| = 1$.
