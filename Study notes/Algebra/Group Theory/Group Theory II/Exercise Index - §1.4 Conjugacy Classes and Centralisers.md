...
- [[Ex - A normal subgroup is a union of conjugacy classes]] (⭐⭐) — prove a subgroup is normal if and only if it is conjugation-saturated, by reading the definition $gNg^{-1}=N$ as a fixed-set condition for the conjugation action; the principle behind every conjugacy-counting (non-)simplicity proof ([[Def - Normal Subgroup]], [[Def - Subgroup]], [[Def - Conjugacy Class]], [[Def - Simple Group]], [[Thm - Lagrange's Theorem]])
