...
> [!note]- Hint 3
> For the $n$-cycle: once orbit-stabiliser gives $|C_{S_n}(\sigma)| = n$, notice that $\sigma$ certainly commutes with each of its own powers $e, \sigma, \sigma^2, \dots, \sigma^{n-1}$ — and there are exactly $n$ of them. A [[Def - Subgroup|subgroup]] of order $n$ containing $n$ known elements *is* those elements. For $(1\,2)(3\,4) \in S_4$: the count gives a centraliser of order $8$; think about which order-$8$ [[Def - Subgroup|subgroup]] of $S_4$ fixes this element under conjugation.
