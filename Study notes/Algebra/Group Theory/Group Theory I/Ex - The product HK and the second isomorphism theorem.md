...
2. **Build a homomorphism to expose structure** (operation 3). The decisive construction is the restricted quotient map $\varphi = \pi|_H : H \to G/N$. The quotient map $G \to G/N$ is one of the standard stock homomorphisms; restricting it to $H$ is the creative twist.
...
> [!note]- Hint 3
> Let $\varphi = \pi|_H : H \to G/N$. Work out $\operatorname{im}\varphi$ — which cosets of $N$ are hit by elements of $H$? — and $\ker\varphi$ — which elements of $H$ map to the identity coset $N$? Then apply the [[Thm - First Isomorphism Theorem|first isomorphism theorem]] to $\varphi$.

> [!note]- Hint 4
> For part 4: the [[Thm - First Isomorphism Theorem|first isomorphism theorem]] gives an isomorphism of *finite* groups, so they have equal order. Use $|HN/N| = |HN|/|N|$ and $|H/(H\cap N)| = |H|/|H\cap N|$, both instances of [[Thm - Lagrange's Theorem|Lagrange]], and solve for $|HN|$.
...
The cosets of $N$ that are hit by elements of $H$ are exactly the cosets $hN$ for $h \in H$, and these are precisely the cosets making up $HN/N$. So $\operatorname{im}\varphi = HN/N$.
