...
Let $G$ be a finite group, and suppose $G$ has **exactly one** element $x$ of order $2$. Show that $x$ commutes with every element of $G$ — that is, $gx = xg$ for all $g \in G$.

**Recall:**

In a group $G$ with identity $e$, the **order** of an element $y$ is the least positive integer $n$ with $y^n = e$. An element $y$ has **order $2$** precisely when $y \neq e$ and $y^2 = e$ — equivalently, $y$ is its own inverse but is not the identity. See [[Def - Order of a Group and of an Element]].
