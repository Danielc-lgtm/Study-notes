...
There is one design decision still to justify: **left** cosets $gH$ versus **right** cosets $Hg = \{hg : h \in H\}$. Both work — both give partitions of $G$ into blocks of size $|H|$, and both prove Lagrange. The reason we cannot simply ignore the distinction is that for a *non-abelian* group the left coset $gH$ and the right coset $Hg$ are generally **different sets**: in $S_3$ with $H = \langle(1\,2)\rangle$, the left and right cosets of a $3$-cycle do not coincide. The subgroups for which $gH = Hg$ for all $g$ are exactly the [[Def - Normal Subgroup|normal subgroups]], and that coincidence is precisely what makes the quotient construction possible — so the left/right distinction is not pedantry, it is the seed of normality. We standardize on left cosets; everything has a mirror-image right-coset version.
