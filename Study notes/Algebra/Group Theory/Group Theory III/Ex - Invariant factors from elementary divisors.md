...
**Theorem routing.** Only the [[Thm - Chinese Remainder Theorem for Cyclic Groups|Chinese remainder theorem]] is used, and only in its fusing direction: it certifies that a product of prime-power cyclic [[Def - Group|groups]] whose orders are *pairwise coprime* equals a single cyclic group of the product order. Each invariant factor $d_i$ will be manufactured by fusing one prime-power factor from each prime, and the divisibility chain $d_{i+1} \mid d_i$ is then forced by how the prime powers are sorted.
