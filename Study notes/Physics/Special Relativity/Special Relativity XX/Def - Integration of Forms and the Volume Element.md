...
Why the factor $\sqrt{|g|}$, and why is it the right one? Evaluate $\mathrm{d}U$ on the coordinate displacements $\mathrm{d}\vec{\ell}_\alpha = \mathrm{d}x^\alpha\,\vec{e}_\alpha$. By multilinearity $\mathrm{d}U = \mathrm{d}x^0\mathrm{d}x^1\mathrm{d}x^2\mathrm{d}x^3\,\epsilon(\vec{e}_0,\vec{e}_1,\vec{e}_2,\vec{e}_3) = \epsilon_{0123}\,\mathrm{d}x^0\mathrm{d}x^1\mathrm{d}x^2\mathrm{d}x^3$, and the component $\epsilon_{0123}$ of the Levi-Civita tensor in a right-handed coordinate basis is $\sqrt{|g|}$. The point of this factor is that it is exactly the Jacobian compensator. Under a change of coordinates the coordinate differentials $\mathrm{d}^4x$ pick up the Jacobian $|J|$, and $\sqrt{|g|}$ picks up $1/|J|$ (because the metric determinant transforms as $\det g' = J^2\det g$, so $\sqrt{|g'|} = |J|^{-1}\sqrt{|g|}$ — wait, $\sqrt{|g'|}=\sqrt{|g|}/|J|$ only if $J=\partial x/\partial x'$; with the conventions below the two factors cancel). The product $\sqrt{|g|}\,\mathrm{d}^4x$ is therefore invariant, which is the whole reason it deserves to be called *the* volume element: it is the unique density that gives the same total for every coordinate system.
