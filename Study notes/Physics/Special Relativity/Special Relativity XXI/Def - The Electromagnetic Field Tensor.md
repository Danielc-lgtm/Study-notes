...
> The identification $F = dA$ makes $F$ the **curvature** of a connection on a $U(1)$ principal bundle, with $A$ the connection one-form; $dF = 0$ is the **Bianchi identity**. Electromagnetism is the abelian case of **Yang–Mills theory**, and the non-abelian generalisation — replacing $U(1)$ by $SU(2)$ or $SU(3)$, so that $F = dA + A\wedge A$ acquires a self-interaction term — is the foundation of the electroweak and strong interactions.
