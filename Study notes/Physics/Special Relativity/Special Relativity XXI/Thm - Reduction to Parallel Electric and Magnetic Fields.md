...
>
> > [!note]- Full proof
> > Since $\mathbf{e}\parallel\mathbf{E}\times\mathbf{B}$ is normal to the plane $P = \mathrm{span}(\mathbf{E},\mathbf{B})$, both fields are transverse to the boost: $\mathbf{E}_\parallel = \mathbf{B}_\parallel = 0$, $\mathbf{E}_\perp = \mathbf{E}$, $\mathbf{B}_\perp = \mathbf{B}$. The transformation law gives $\mathbf{E}' = \Gamma(\mathbf{E} + \mathbf{U}\times\mathbf{B})$, $\mathbf{B}' = \Gamma(\mathbf{B} - \tfrac{1}{c^2}\mathbf{U}\times\mathbf{E})$. Now $\mathbf{U}\parallel\mathbf{E}\times\mathbf{B}$, so the plane perpendicular to $\mathbf{U}$ is exactly $P$; the vector $\mathbf{U}\times\mathbf{B}$ is perpendicular to $\mathbf{U}$, hence lies in $P$, and likewise $\mathbf{U}\times\mathbf{E}\in P$. Therefore $\mathbf{E}', \mathbf{B}'\in P$: the boosted fields stay in the original field plane. $\blacksquare$
