...
> [!tip] The Wigner Classification — Why Every Particle Has a Mass and a Spin *(from QFT and Representation Theory)*
> The deepest payoff of the Lorentz group is an answer to a question that looks like physics but is really group theory: *why does every elementary particle carry exactly two labels, a mass and a spin, and nothing else?* The reasoning is the **Wigner classification**. A quantum state of a single particle must transform under the symmetries of spacetime, so it must carry a representation of the Poincaré group; "elementary" means the representation is *irreducible* — it has no smaller invariant piece — and "quantum" means it must be *unitary*, since probabilities are preserved. Wigner classified all the irreducible unitary representations of the Poincaré group, and the answer is that they are labelled by precisely two invariants: the value of $P^\mu P_\mu$, which is the **mass** squared, and a label coming from the little group that fixes a chosen momentum, which is the **spin**. There is no third label, and that is why mass and spin exhaust the intrinsic identity of a particle. The classification also explains where half-integer spin comes from. The proper orthochronous Lorentz group $SO^+(1,3)$ is not simply connected; its **double cover** is the group $SL(2,\mathbb{C})$ of $2\times 2$ complex matrices of determinant one, and a representation of the double cover need only return to itself after a $4\pi$ rotation rather than $2\pi$. Representations that descend to $SO^+(1,3)$ itself carry integer spin — scalars, vectors, tensors; the genuinely new representations of $SL(2,\mathbb{C})$, the spinors, carry half-integer spin, and electrons exist because the Lorentz group has a double cover. This is the organising principle of **quantum field theory**: fields are sorted by which representation of $SL(2,\mathbb{C})$ they carry, and that representation is their spin.

> [!tip] Why a Particle Is Exactly a Mass and a Spin *(from Representation Theory and QFT)*
> Pursue the **Wigner classification** one step further than the previous note, to the question of why the list of labels closes at exactly two. The argument is pure group theory wearing physical clothing. A single-particle quantum state lives in a Hilbert space on which the Poincaré group — the Lorentz group together with spacetime translations — must act, since the choice of inertial frame and origin is physically arbitrary; the action must be by *unitary* operators, because a change of frame cannot alter probabilities; and "elementary" is the demand that the representation be *irreducible*, with no proper subspace closed under the group. Wigner's theorem enumerates all irreducible unitary representations of the Poincaré group, and the enumeration produces precisely two invariants. The first is the value of the Casimir $P^\mu P_\mu$, the Lorentz-invariant norm of the four-momentum, which is the **mass** squared — invariant because it is a four-vector contraction. The second emerges from the *little group*: fix a reference momentum (the rest momentum for a massive particle), and the subgroup of Lorentz transformations leaving it fixed is a copy of the rotation group $SO(3)$; the irreducible representations of that little group are labelled by a single number, and that number is the **spin**. There is no room for a third label, and that absence is the content of the statement that mass and spin exhaust a particle's intrinsic identity — everything else about a particle is interaction, not kinematics. The appearance of *half-integer* spin is the final twist, and it is geometric. The proper orthochronous Lorentz group $SO^+(1,3)$ is not simply connected — a $2\pi$ rotation is a loop that cannot be contracted to a point — and its universal **double cover** is $SL(2,\mathbb{C})$, the group of $2\times2$ complex matrices of determinant one. The little group's double cover is correspondingly $SU(2)$ rather than $SO(3)$, and $SU(2)$ has representations of every half-integer dimension. Representations that factor through $SO(3)$ itself give integer spin — scalars, vectors, tensors, which return to themselves under a $2\pi$ rotation; the genuinely new representations of $SU(2)$, the spinors, give half-integer spin and acquire a minus sign under $2\pi$, returning to themselves only after $4\pi$. Electrons, with spin $\tfrac12$, exist because the Lorentz group has a double cover; the structure fixed on this page — $O(1,3)$ and its connected piece $SO^+(1,3)$ — is, through $SL(2,\mathbb{C})$, the reason every particle in nature carries one mass and one spin and no other intrinsic label.
