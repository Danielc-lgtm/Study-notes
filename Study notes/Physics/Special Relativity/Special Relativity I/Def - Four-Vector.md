...
> [!tip] The Tangent Bundle and General Coordinate Transformations *(from Differential Geometry and General Relativity)*
> Defining a four-vector by its transformation law, rather than by being a list of numbers, is precisely the move that survives the passage to a curved manifold. On a general manifold the four-vectors based at an event $p$ form the **tangent space** $T_p$, and assembling these spaces over all events gives the **tangent bundle** — a four-vector field is then a *section* of that bundle, a smooth choice of tangent vector at every point. The only thing that changes is the transformation law. In special relativity the coordinate change between inertial frames is linear, so its Jacobian is the *constant* matrix $\Lambda^\mu{}_\nu$, the same at every event, and the law reads $V'^\mu = \Lambda^\mu{}_\nu V^\nu$. On a manifold the coordinate change $x \mapsto x'(x)$ is an arbitrary smooth map, so its Jacobian — the matrix of partial derivatives [[Def - Partial Derivatives and the Jacobian Matrix|computed in multivariate analysis]] — is *position-dependent*, and the transformation law becomes $V'^\mu = (\partial x'^\mu/\partial x^\nu)\, V^\nu$. The constant $\Lambda^\mu{}_\nu$ is just this Jacobian frozen at its (everywhere-identical) value for a linear map; a four-vector is the special-relativistic shadow of a general tangent vector, and the Lorentz transformation law is the general coordinate transformation law specialised to the case where the Jacobian does not move. This is exactly why the "transforms correctly" definition is the right one to enshrine: it is the one that does not break when spacetime curves.

> [!tip] From the Lorentz Matrix to the Jacobian — Vectors on a Manifold *(from Differential Geometry and General Relativity)*
> The transformation law $V'^\mu = \Lambda^\mu{}_\nu V^\nu$ should be read not as a special formula but as the simplest instance of a single universal rule, and seeing the general rule is what carries this definition into differential geometry. On a smooth manifold there is no preferred class of coordinate systems and no preferred linear transformations between them: a change of coordinates is an *arbitrary* smooth invertible map $x \mapsto x'(x)$. Its derivative is the [[Def - Partial Derivatives and the Jacobian Matrix|Jacobian matrix]] of partial derivatives $\partial x'^\mu/\partial x^\nu$, and a tangent vector — the velocity $dx^\mu/d\lambda$ of a curve, the prototype four-vector — transforms by exactly that Jacobian, $V'^\mu = (\partial x'^\mu/\partial x^\nu)\,V^\nu$. This is just the chain rule. Now place special relativity inside this picture. Inertial coordinate systems are related by Lorentz transformations, which are *linear* maps; the Jacobian of a linear map is the constant matrix of its coefficients, the same at every event, and that constant matrix is precisely $\Lambda^\mu{}_\nu$. So the Lorentz transformation law is the general manifold transformation law in the special case where the Jacobian does not vary from point to point — where spacetime is flat enough that one coordinate system covers it all and the transition maps are global linear maps. The four-vectors based at an event $p$ then form the **tangent space** $T_p$, the assembly of these spaces over all events is the **tangent bundle**, and a four-vector *field* is a section of that bundle, a smooth choice of tangent vector at every point. Nothing about the *definition* changes in the passage to a manifold — a four-vector is still "the thing that transforms correctly" — only the transformation matrix is upgraded from the rigid global $\Lambda$ to the position-dependent Jacobian. That this single sentence survives the upgrade is the whole reason the transformation-law definition, rather than the naive list-of-numbers definition, is the one worth enshrining.
