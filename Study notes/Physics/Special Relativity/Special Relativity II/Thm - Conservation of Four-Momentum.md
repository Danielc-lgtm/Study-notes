...
> [!tip] Mandelstam Variables and Collider Kinematics *(from Particle Physics)*
> Conservation of four-momentum is the working tool of all collider physics, and for a two-body scattering process $1 + 2 \to 3 + 4$ it is packaged into three Lorentz-invariant numbers, the **Mandelstam variables**: $s = (P_1 + P_2)^2$, $t = (P_1 - P_3)^2$, and $u = (P_1 - P_4)^2$. The first is the square of the total four-momentum — the invariant mass squared of the system — and in the centre-of-momentum frame it equals the centre-of-mass energy squared, so $\sqrt{s}$ is the headline energy of a collider. The other two are momentum transfers carrying the scattering-angle information. They are not independent: because each external four-momentum is on its mass shell, $P_i\cdot P_i = m_i^2$, and because conservation of four-momentum gives $P_1 + P_2 = P_3 + P_4$, expanding the three squares and summing makes every cross term cancel, leaving the constraint $s + t + u = m_1^2 + m_2^2 + m_3^2 + m_4^2$. Two of the three variables therefore fix the kinematics completely. The scattering amplitude written as a function $\mathcal{M}(s,t,u)$ then displays **crossing symmetry**: the same analytic function describes the $s$-channel, $t$-channel and $u$-channel reactions, related by continuing one incoming particle to an outgoing antiparticle. See [[Ex - Mandelstam variables for two-body scattering]].

> [!tip] The Mandelstam Sum Rule $s + t + u = \sum m_i^2$ *(from Particle Physics)*
> The single most-used algebraic fact about the **Mandelstam variables** is the *sum rule*, and it is a direct corollary of this theorem. For $1+2\to 3+4$, expand each of $s = (P_1+P_2)^2$, $t = (P_1-P_3)^2$, $u = (P_1-P_4)^2$ by the Minkowski binomial formula: every term of the form $P_i\cdot P_i$ becomes the mass-shell value $m_i^2$, and the cross terms are $+2P_1\cdot P_2$, $-2P_1\cdot P_3$, $-2P_1\cdot P_4$. Adding the three, the cross terms collect into $2P_1\cdot(P_2 - P_3 - P_4)$, and conservation of four-momentum, $P_1 + P_2 = P_3 + P_4$, says exactly that $P_2 - P_3 - P_4 = -P_1$, so the bracket is $-P_1$ and the cross terms collapse to $-2P_1\cdot P_1 = -2m_1^2$. What survives is $s + t + u = (m_1^2 + m_2^2) + (m_1^2 + m_3^2) + (m_1^2 + m_4^2) - 2m_1^2 = m_1^2 + m_2^2 + m_3^2 + m_4^2$. The sum of the three invariants is therefore fixed by the external masses alone — it carries no dynamical information — so a two-body process has only *two* independent kinematic invariants, and a scattering amplitude is genuinely a function $\mathcal{M}(s,t)$ on a two-dimensional region. This is the constraint that underlies the Mandelstam plane and the analytic structure exploited by crossing symmetry. See [[Ex - Mandelstam variables for two-body scattering]].
