...
- [[Ex - Stress-Energy Tensor of Dust]] (⭐) — Verify that $T^{\mu\nu} = \rho u^\mu u^\nu$ has the correct rest-frame components and that conservation $\nabla_\mu T^{\mu\nu} = 0$ unpacks (via projection along and orthogonal to $u^\mu$) into the continuity equation and the geodesic equation. ([[Def - Stress-Energy Tensor]], [[Def - Four-Vector]], [[Def - Spacetime Manifold]])
