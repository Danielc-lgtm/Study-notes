...
> [!note]- 🔵 Time-Bounded AIT / Universal Search / Algorithmic Meta-Learning (Levin, Hutter, Schmidhuber)
> **Prereqs:** AIT / AIXI / Reflective Oracles (Kolmogorov complexity, Solomonoff induction, universal priors), Computational Complexity (time hierarchies, average-case hardness), Computability, 🟢 Information Theory; Combinatorial Optimization / Algorithm Design helps for the practical program-search track
>
> **Connects:** AIT ↔ Computational Complexity (meta-complexity: MCSP, MKtP) ↔ Combinatorial Optimization / Algorithm Design (program search as discrete search) ↔ Algorithmic Statistics (resource-bounded MDL and sufficient statistics) ↔ Stochastic Thermodynamics (logical depth as the compute-cost face of algorithmic thermodynamics) ↔ Fixed Point Theorems (Gödel-machine self-reference)
>
> **Note:** What algorithmic information theory becomes when programs must also be *fast*: replace "shortest program" by "shortest-and-fastest program" and every classical AIT object acquires a resource-bounded twin. The organizing quantity is Levin complexity $Kt(x) = \min_p \{|p| + \log \mathrm{time}(p)\}$, and the organizing algorithm is Levin's universal search: run all programs in parallel with time share $2^{-|p|}$, which solves any invertible problem in time $2^{Kt(\mathrm{solution})}$ up to a multiplicative constant — asymptotically optimal among all search orders. Schmidhuber's speed prior $S$ is the induction-side counterpart: reweight Solomonoff's universal prior by computation time so that prediction becomes limit-computable and fast-computable hypotheses dominate. On top of these two primitives sits the meta-learning tower: AIXI-tl (the time/length-bounded universal agent), Hutter's HSEARCH and the fastest-and-shortest-algorithm theorem, OOPS (bias-optimal incremental search that *reuses previously found subprograms* — transfer learning as program-library growth), PowerPlay (jointly searches over self-invented problems and solver modifications, generating an automatic curriculum), and the Gödel machine (provably beneficial self-rewrites via self-reference). The rigorous complexity-theory face of the same subject is modern meta-complexity: the Minimum Circuit Size Problem, MKtP, and the Liu-Pass theorem that one-way functions exist if and only if time-bounded Kolmogorov complexity is mildly hard on average. Directly relevant to the ontology-identification programme: a concept-as-reusable-function-pointer is precisely an OOPS/library-learning subprogram whose amortized $Kt$-cost drops with reuse, and time-bounded AIT is the setting where that becomes a theorem rather than a metaphor. Member of Cluster 16; the logical-depth / thermodynamics track also ties to Cluster 11.
>
> **Coverage checklist (concepts the Obsidian pages must include):** Levin complexity $Kt$; Levin universal search / LSEARCH and bi-optimality; time-bounded universal distribution $m^t$ and time-bounded Solomonoff induction; the speed prior $S$ and speed-prior-based prediction; AIXI-tl; HSEARCH / the fastest-and-shortest-algorithm theorem; adaptive Levin search; OOPS (optimal ordered problem solver, bias-optimality, backtracking over program prefixes, subprogram reuse); PowerPlay (self-invented problems, automatic curricula); the Gödel machine and the success-story algorithm; compression progress / formal theory of curiosity and intrinsic motivation; resource-bounded Kolmogorov complexity variants ($K^t$, $Kt$, $KT$, space-bounded, distinguishing complexity $CD^t$); Bennett's logical depth and computational depth (Antunes-Fortnow-van Melkebeek-Vinodchandran); meta-complexity (MCSP, MKtP, hardness magnification, Hirahara's worst-case-to-average-case reductions); Liu-Pass one-way functions ↔ average-case hardness of $K^t$; Levin's optimal inverter for one-way functions; time-bounded MDL and sophistication; incremental compression (Franz et al. — compressing by stacking features $f \circ f' \circ \cdots$, each found by a tractable search, with guarantees on when the greedy feature-by-feature route reaches the Kolmogorov optimum); library learning / DreamCoder-style wake-sleep program induction as the modern practical face of subprogram reuse
>
> **Unlocks:** A rigorous foundation for meta-learning and transfer (learning-to-learn as search over search strategies, with reuse theorems), the ontology-identification research programme (reusable function pointers as amortized-$Kt$ library entries), bounded-rationality agent foundations (resource-bounded AIXI variants), algorithmic thermodynamics with explicit compute costs (depth as work — the Maxwell's-demon tie), and cryptography from meta-complexity (one-way functions from $K^t$ hardness)
>
> **Key refs:** Li-Vitányi, *An Introduction to Kolmogorov Complexity and Its Applications* (4th ed., Springer 2019), Chapter 7 (resource-bounded complexity); Levin, *Universal Sequential Search Problems* (Probl. Inform. Transm. 9, 1973); Schmidhuber, *The Speed Prior* (COLT 2002), *Optimal Ordered Problem Solver* (Machine Learning 54, 2004), and *PowerPlay* (Frontiers in Psychology 4:313, 2013); Hutter, *Universal Artificial Intelligence* (Springer 2005) for AIXI-tl and *The Fastest and Shortest Algorithm for All Well-Defined Problems* (IJFCS 13, 2002); Liu-Pass, *On One-Way Functions and Kolmogorov Complexity* (FOCS 2020) — the meta-complexity gateway; Franz-Antonenko-Soletskyi, *A Theory of Incremental Compression* (arXiv:1908.03781) — decomposes universal compression into a sequence of feature extractions, the closest existing formal treatment of the reusable-function-pointer picture
...
> [!note]- 🔵 Perturbation Methods / Asymptotic Analysis / Integral Transforms (4,9)
> **Prereqs:** 🟢 PDE, Complex Analysis (steepest descent, Stokes lines), 🟢 Functional Analysis, 🟢 ODEs; Dynamical Systems helps for the averaging / normal-form track
>
> **Connects:** PDE ↔ Complex Analysis (asymptotics of integrals live on deformed contours) ↔ Dynamical Systems (averaging, Poincaré-Lindstedt, normal forms) ↔ Microlocal / Semiclassical Analysis (WKB made rigorous) ↔ Renormalization Group (the Chen-Goldenfeld-Oono RG method subsumes multiple scales and boundary layers) ↔ Continuum Mechanics / Fluid Dynamics (Prandtl boundary layers, lubrication theory) ↔ QFT (divergent perturbation series, Borel summation, resurgence)
>
> **Note:** The systematic art of controlled approximation: expand in a small parameter $\varepsilon$, and — the actual content of the subject — know *where* and *why* the expansion is valid, since every method is defined by its failure mode. **Regular perturbation** fails when $\varepsilon$ multiplies the highest derivative or when secular terms grow in time, and each failure has its named repair. **Singular perturbation / matched asymptotics:** boundary layers, inner and outer expansions, distinguished limits and Van Dyke matching. **Multiple scales, Poincaré-Lindstedt, and averaging:** remove secular growth by letting frequency or slow time be part of the unknown; the averaging theorem is the rigorous dynamical-systems face. **WKB / semiclassical expansion:** $\psi \sim e^{S/\varepsilon}$ ansatz, turning points, Airy connection formulas — made rigorous in the Microlocal node. **Asymptotics of integrals:** Watson's lemma, Laplace's method, stationary phase, steepest descent — the workhorse trio behind every transform inversion, with the Stokes phenomenon (asymptotic representations jump discontinuously across Stokes lines) as the fundamental subtlety, smoothed by Berry and systematized by resurgence and trans-series (Écalle) — the modern frontier where exponentially small terms are bookkept exactly, feeding back into QFT via Borel summation of divergent series. **Homogenization:** two-scale expansions for periodic media, the PDE-theoretic face of averaging. **Integral transforms:** Laplace, Fourier, and Mellin transforms as the exact-solution substrate the asymptotic methods act on. An asymptotic series is a different object from a convergent one — optimal truncation at the least term, accuracy $e^{-c/\varepsilon}$, is the quantitative statement — and internalizing that distinction is the single highest-leverage insight of the subject. Member of Cluster 18; the WKB / stationary-phase track ties to Cluster 5, and the RG-asymptotics and resurgence tracks tie to Cluster 2 and Cluster 19.
>
> **Unlocks:** Rigorous semiclassics (via Microlocal Analysis), exponential asymptotics and resurgence / trans-series, homogenization theory for composite and porous media, the RG approach to asymptotics (Chen-Goldenfeld-Oono), Prandtl boundary-layer theory and lubrication approximations in fluids, saddle-point evaluation of partition functions and path integrals, and the validity-conditions discipline that powers the approximation-methods exercise track
>
> **Key refs:** Bender-Orszag, *Advanced Mathematical Methods for Scientists and Engineers* (Springer, 1999) — the canonical text; Hinch, *Perturbation Methods* (Cambridge, 1991) — the concise practitioner's course; Holmes, *Introduction to Perturbation Methods* (Springer, 2nd ed., 2013) — includes homogenization and multiple scales done carefully; Miller, *Applied Asymptotic Analysis* (AMS GSM 75, 2006) — the integrals-and-PDE side with full rigor
...
**Members:** AIT / AIXI / Reflective Oracles · Time-Bounded AIT / Universal Search / Algorithmic Meta-Learning · Computability · Computational Complexity · Computable Analysis · Arithmetic Hierarchy · Combinatorial Optimization / Algorithm Design · Fixed Point Theorems
...
**Members:** Green’s Functions · Perturbation Methods / Asymptotic Analysis / Integral Transforms · Finite Element Exterior Calculus · Computational Electromagnetics (Discrete Exterior Calculus) · Convex Optimization / High-Dim Optimization · Linear Matrix Inequalities · Optimal Filtering / State Estimation · Path Integrals for Filtering and Control · Detection and Estimation Theory · System Identification · Networked Control Systems · Robust Control / H∞ · Dimensionality Reduction / Compressed Sensing · Linear Systems Theory / Circuit Theory / Signal Processing · Electrical Circuit Theory
