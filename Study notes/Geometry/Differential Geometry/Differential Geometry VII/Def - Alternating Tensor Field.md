...
The vanishing identity has a striking dimension consequence: $\Lambda^k(V^*) = 0$ for $k > n$, because every $k$-tuple of vectors in an $n$-dimensional space with $k > n$ is linearly dependent. The non-trivial [[Def - Dimension|dimensions]] are $\dim \Lambda^k(V^*) = \binom{n}{k}$ for $0 \leq k \leq n$, with $\Lambda^n(V^*)$ being a 1-dimensional "top-form" space — the home of the determinant and of [[Def - Volume Form|volume forms]].
...
**Is NOT an instance: a $(0, k)$-tensor with $k > n$ on an $n$-manifold.** Such a tensor field has $\binom{n}{k} = 0$ independent components for $k > n$, so $\Lambda^k(T^*M) = 0$ — there is no nonzero alternating $k$-form for $k > n$. (Note: general covariant $k$-tensor fields for $k > n$ are *not* zero — there are $n^k$ components — but the alternating [[Def - Subspace|subspace]] is trivial.)
