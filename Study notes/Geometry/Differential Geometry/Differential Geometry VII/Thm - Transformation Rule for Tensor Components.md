...
The terminology is reversed from naive expectation because "covariant" historically meant "transforms like the basis", and basis vectors transform with $\partial \tilde x^i / \partial x^a$ — wait, that is in the *opposite* direction. The terminology is unfortunately backwards from modern usage; the rule is what is operationally important, the naming is a historical accident.
...
A second common error is confusing whether a quantity is a tensor or a pseudo-tensor. Volume forms and the Levi-Civita symbol $\epsilon_{ijk}$ are pseudo-tensors: they transform by the rule with an extra $\operatorname{sgn}(\det J)$ factor, which is $+1$ for orientation-preserving and $-1$ for orientation-reversing chart changes. On orientable manifolds with a chosen orientation, pseudo-tensors and tensors agree; on non-orientable manifolds, the distinction is real.
...
- **Pseudo-tensors and tensor densities.** A *pseudo-tensor* of weight $w$ transforms by the rule with an extra factor of $(\det J)^w$. Densities (e.g., the volume element $\sqrt{|\det g|}\, dx^1\wedge\cdots\wedge dx^n$) are pseudo-tensors of weight 1 — they pick up a Jacobian determinant under change of coordinates. The transformation rule extends naturally to pseudo-tensors with the extra Jacobian factor.
...
> The Levi-Civita symbol $\epsilon_{ijk}$ defined by $\epsilon_{123} = 1$ and antisymmetric — is *not* a tensor: under a change of coordinates with Jacobian $J$, $\tilde\epsilon_{ijk} = (\det J) \cdot \epsilon_{ijk}$ rather than $\epsilon_{ijk}$ (without the determinant). So $\epsilon$ is a pseudo-tensor of weight 1. To make a true tensor, divide by $\sqrt{|\det g|}$ (the Riemannian volume): the resulting object $\epsilon^*_{ijk} = \sqrt{|\det g|}\,\epsilon_{ijk}$ is a true alternating $(0, 3)$-tensor — the Riemannian volume form's components.
