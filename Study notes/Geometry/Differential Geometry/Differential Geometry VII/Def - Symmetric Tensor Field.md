...
The axiom is a single requirement: $\alpha(v_{\sigma(1)}, \dots, v_{\sigma(k)}) = \alpha(v_1, \dots, v_k)$ for every permutation $\sigma \in S_k$. This is a strong condition — it cuts the $n^k$-dimensional space $T^k(V^*)$ down to a $\binom{n+k-1}{k}$-dimensional subspace, since a symmetric tensor is determined by its values on *unordered* $k$-tuples of basis vectors, and the number of such unordered tuples is the multinomial coefficient counting multisets.
...
**Dimension of the fibre.** With $n = \dim V$,
