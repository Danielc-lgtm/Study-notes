...
- **Diffeomorphism invariance / change of variables.** For an orientation-preserving diffeomorphism $F : N \to M$, $\int_M\omega = \int_N F^*\omega$. For orientation-reversing $F$, $\int_M\omega = -\int_N F^*\omega$ (see [[Thm - Change of Variables for Integration on Manifolds]]).
