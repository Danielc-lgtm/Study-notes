...
The structure-group perspective generalizes: for any rank-$n$ real vector bundle $E \to M$ over a manifold, an *orientation of $E$* is a reduction of its structure group from $\mathrm{GL}(n, \mathbb{R})$ to the index-2 subgroup $\mathrm{GL}_+(n, \mathbb{R})$. A manifold orientation is exactly this reduction for $E = TM$.
