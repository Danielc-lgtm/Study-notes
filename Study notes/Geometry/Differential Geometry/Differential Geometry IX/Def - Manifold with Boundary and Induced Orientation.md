...
A **smooth manifold with boundary** is then defined exactly as a smooth manifold, but using $\mathbb{H}^n$ instead of $\mathbb{R}^n$ as the local model. Charts are homeomorphisms $\varphi : U \to \widehat U \subseteq \mathbb{H}^n$ with smooth transitions in the half-space sense (smooth on the closure of an open subset, with all derivatives extending continuously). The **boundary** $\partial M$ is the set of points sent to $\{x^n = 0\}$ by any (equivalently, every) chart — well-defined because boundary points cannot be interior in any chart, as the half-space model is local-diffeomorphism-distinct from $\mathbb{R}^n$ at boundary points.
