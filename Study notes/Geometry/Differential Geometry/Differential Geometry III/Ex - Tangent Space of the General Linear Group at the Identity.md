...
Let $\mathrm{GL}(n, \mathbb{R}) \subseteq M_{n}(\mathbb{R})$ denote the general linear group — the set of invertible $n \times n$ real matrices, regarded as an open subset of the vector space $M_{n}(\mathbb{R}) \cong \mathbb{R}^{n^{2}}$ (the open subset where the determinant is nonzero).
...
For part (c), the key step is recognizing that the second-order expansion captures the Lie bracket — that the *failure of commutativity* of one-parameter subgroups is encoded at second order, not first. This is the geometric source of the commutator $[A, B]$ on $\mathfrak{gl}(n)$.
