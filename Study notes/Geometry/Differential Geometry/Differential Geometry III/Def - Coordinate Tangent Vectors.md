...
The basis property — that $\partial/\partial x^{1}|_{p}, \dots, \partial/\partial x^{n}|_{p}$ span $T_{p}M$ — follows from [[Thm - Dimension of the Tangent Space]] via the chart isomorphism. The vectors are linearly independent (apply them to the coordinate functions $x^{j}$ and get the identity matrix), and there are $n$ of them, matching the dimension of $T_{p}M$. So they are a basis.
