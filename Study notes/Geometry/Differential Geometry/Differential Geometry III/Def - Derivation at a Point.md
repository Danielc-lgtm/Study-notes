...
**Is NOT an instance: evaluation at $p$.** The map $\mathrm{ev}_{p}(f) = f(p)$ is linear but does *not* satisfy the Leibniz rule at $p$: $\mathrm{ev}_{p}(fg) = f(p)g(p)$, while $f(p)\,\mathrm{ev}_{p}(g) + g(p)\,\mathrm{ev}_{p}(f) = 2f(p)g(p)$. The two agree only when $f(p)g(p) = 0$. So evaluation at $p$ is a [[Def - Ring|ring]] homomorphism but not a derivation — it sees the *value* of $f$, not its first-order behaviour.
