...
The resulting smooth structure on $TM$ has a critical feature: it is **locally trivial**. Every chart $(U, \varphi)$ on $M$ gives a diffeomorphism $\pi^{-1}(U) \cong U \times \mathbb{R}^{n}$, the chart restricted to the manifold piece combined with the fibre identification. Globally, $TM$ need not be a product $M \times \mathbb{R}^{n}$ — that is the genuine geometric content.
...
**Local triviality.** For each chart $(U, \varphi)$, the natural chart $\tilde\varphi$ provides a diffeomorphism
