...
Why use *all* smooth functions $f$ rather than just coordinate functions? Because using only coordinate functions would make the equivalence relation chart-dependent — two curves equivalent in one chart might not be in another. Using all smooth functions is intrinsic. The trade-off is that the definition is harder to *check* (you cannot verify the equivalence by inspecting a finite collection of derivatives) but easier to *use in proofs* (the equivalence is manifestly chart-independent).
