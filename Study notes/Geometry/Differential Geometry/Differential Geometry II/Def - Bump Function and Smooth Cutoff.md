...
This function is *smooth but not analytic at $0$*: its Taylor series at $0$ is $0 + 0t + 0t^2 + \ldots$, which converges to $0$ everywhere — but $\psi_0$ is not zero on $(0, \infty)$. The Taylor series at $0$ fails to recover $\psi_0$. This non-analyticity is *the property that makes bump functions possible*: it is precisely the slack between $C^\infty$ and $C^\omega$ that lets us build cutoffs. **There are no analytic bump functions** (an analytic function vanishing on an open set vanishes everywhere by analytic continuation), so the entire local-to-global machinery of differential geometry rests on the $C^\infty$-vs-$C^\omega$ gap.
...
**Is an instance: bump along an axis.** $\psi : \mathbb{R} \to [0, 1]$, $\psi(t) = h(t)h(-t)$ where $h$ is a one-sided cutoff vanishing on $[1, \infty)$ and equal to $1$ on $(-\infty, 0]$. This is a smooth function equal to $1$ in a neighbourhood of $0$ and supported in $(-1, 1)$. Useful for products: a bump along the $i$th axis $\mathbb{R}^n \to [0, 1]$, $(x^1, \ldots, x^n) \mapsto \psi(x^i)$.
