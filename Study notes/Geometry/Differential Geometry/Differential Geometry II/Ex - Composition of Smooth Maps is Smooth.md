...
This exercise sets the routine for every smoothness verification — projections, inclusions, quotient maps, group operations on Lie groups — and the move "chart on the target, then shrink the source chart via continuity, then recognize the coordinate composition as smooth" is the constant move throughout differential geometry.
