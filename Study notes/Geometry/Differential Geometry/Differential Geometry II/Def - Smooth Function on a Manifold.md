...
**Is an instance: constant functions.** Every constant function $f \equiv c : M \to \mathbb{R}$ is smooth. In any chart, $\widehat f = c$ is the constant Euclidean function, which is smooth. (The smoothness is the only verification; this is the trivial case but also the base case for $C^\infty(M)$ containing $\mathbb{R}$ as a subring.)
