...
One must distinguish **full faithfulness** from **essential surjectivity**. The functor is faithful because smooth real-valued functions separate points: if $F\neq G$, some $h\in C^\infty(N)$ has $h\circ F\neq h\circ G$. For ordinary finite-dimensional Hausdorff second-countable manifolds, unital $\mathbb R$-algebra homomorphisms $C^\infty(N)\to C^\infty(M)$ are induced by unique smooth maps, so the functor is full on this category. It is not essentially surjective onto all commutative $\mathbb R$-algebras: most algebras are not smooth-function algebras of manifolds. A related recovery test is that a homeomorphism $F:M\to N$ is a [[Def - Diffeomorphism|diffeomorphism]] exactly when pullback carries smooth functions in both directions (equivalently, $F^*$ identifies the two smooth-function algebras).
...
- $L^\infty(X,\mu)$, essentially bounded measurable functions modulo almost-everywhere equality. (For finite $p$, $L^p$ is generally not closed under multiplication, so it is not usually an algebra.)
...
**Is an instance: $C^\infty(\text{point})$ = $\mathbb{R}$.** The smooth manifold consisting of a single point has $C^\infty(\{*\}) = \mathbb{R}$ — every function on a single point is a real number. The one-point manifold is terminal in the manifold category, while $\mathbb{R}$ is initial among unital commutative $\mathbb{R}$-algebras; contravariance reverses these roles.

**Is an instance: $C^\infty(S^1)$.** The ring of smooth functions on the circle is the ring of smooth $2\pi$-periodic functions on $\mathbb{R}$ — and via Fourier series, it is in bijection with smooth functions on $S^1$ identified by their Fourier coefficients. Its real trigonometric polynomials—finite linear combinations of $1$, $\cos(kt)$, and $\sin(kt)$—are dense in its standard Fréchet topology; the complex exponentials $e^{ikt}$ belong instead to the complex-valued smooth-function algebra.
...
**Is NOT the same instance: bounded smooth functions on a noncompact $M$.** The bounded smooth functions do form a unital $\mathbb{R}$-subalgebra of $C^\infty(M)$: multiplication by any fixed real scalar preserves boundedness. It is generally proper—for example, $x\mapsto x$ is excluded on $\mathbb R$—and it is not complete in the supremum norm, because a uniform limit of smooth bounded functions need only be continuous. Its supremum-norm completion is contained in the bounded continuous functions, not in general in the smooth ones.

**Is NOT an instance: the ring of *real-analytic* functions $C^\omega(M)$.** This is a strict subalgebra of $C^\infty(M)$ — every real-analytic function is smooth, but most smooth functions are not analytic. The key distinction is that on each connected component, analytic functions are determined by their germ at any point (analytic continuation), while smooth functions have *independent* values at different points. So $C^\omega(M)$ is dramatically smaller than $C^\infty(M)$, and the analytic category lacks bump functions, partitions of unity, and the extension lemma.
...
**Corollary (infinite-dimensional).** For any nonempty $M$ of [[Def - Dimension|dimension]] at least $1$, $C^\infty(M)$ is infinite-dimensional as an $\mathbb{R}$-vector space. Choose a chart whose image contains an open Euclidean ball. Inside that ball choose countably many pairwise disjoint smaller balls, and pull back a nonzero bump supported in each one. If $\sum_{j=1}^N c_jf_j=0$, evaluate at a point where $f_k$ is nonzero: all other bumps vanish there, so $c_k=0$. Thus every finite subfamily is linearly independent, proving infinite dimensionality. (Lee Problem 2-7.)
