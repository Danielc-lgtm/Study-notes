...
**The same smooth manifold can carry many genuinely different Riemannian metrics.** The open subset $\mathbb{H}^2 \subseteq \mathbb{R}^2$ is a single smooth manifold, but it admits at least two distinct Riemannian metrics — the induced Euclidean metric and the hyperbolic metric. These give different geometries (flat vs. constantly negatively curved), different distance functions, different geodesics, different volumes, different isometry groups. The geometric content of "$(M, g)$" depends on *both* the manifold and the metric, and changing the metric changes everything geometric while leaving the smooth structure intact. The reusable lesson: never assume "the geometry" of a smooth manifold without specifying a metric. The smooth manifold is a chassis; the metric is the engine.
