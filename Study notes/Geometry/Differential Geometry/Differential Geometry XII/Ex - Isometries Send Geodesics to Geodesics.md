...
Let $F : (M, g) \to (N, h)$ be a Riemannian isometry between Riemannian manifolds (so $F$ is a smooth [[Def - Diffeomorphism|diffeomorphism]] with $F^* h = g$). Prove that $F$ sends [[Def - Geodesic|geodesics]] of $g$ to geodesics of $h$ — equivalently, that geodesics are *invariants* of Riemannian [[Def - Isometry|isometries]].
...
**Step 2: [[Def - Geodesic|Geodesics]] of $g$ map to geodesics of $h$.**
...
**Isometries are the same kind of map as [[Def - Group|group]] [[Def - Homomorphism|homomorphisms]] — they preserve the structure that defines the category.** A diffeomorphism preserves the smooth structure but not the metric; an isometry preserves both. In the language of category theory (see [[Def - Isometry of Riemannian Manifolds]]'s Categorical section), isometries are the [[Def - Isomorphism|isomorphisms]] in the category of Riemannian manifolds. Just as a [[Def - Group|group]] homomorphism preserves multiplication and identity (the structure of group), a Riemannian isometry preserves the metric (the structure of Riemannian manifold). Every property of a group that is "structural" — order of elements, conjugacy classes, abelianness — is preserved by [[Def - Homomorphism|homomorphisms]]; every property of a Riemannian manifold that is "structural" — curvature, geodesics, distance — is preserved by isometries. The parallel between algebraic and geometric structure-preserving maps is one of the unifying themes of modern mathematics.
