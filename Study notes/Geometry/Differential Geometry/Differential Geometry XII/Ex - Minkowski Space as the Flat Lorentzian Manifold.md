...
This is the bridge between [[Special Relativity I — Postulates and Lorentz Transformations|special relativity]] and the abstract theory of Lorentzian manifolds in this chapter: the entire content of SR is the geometry of this single flat Lorentzian manifold.
...
**Key decision point.** The non-obvious choice is *which [[Def - Isometry|isometries]] to enumerate*. The candidates are: linear maps preserving $\eta$ (the Lorentz group $O(1, 3)$); translations (which preserve $\eta$ because it is constant); and combinations of the two. The argument that *every* isometry is of this form uses affine rigidity: an isometry preserves the Levi-Civita connection and therefore sends affinely parametrized straight-line geodesics to affinely parametrized straight lines. Its value and differential at one point consequently determine it everywhere. One cannot infer constancy of $dF_p$ merely from $dF_p\in O(1,3)$, because $O(1,3)$ is a positive-dimensional Lie group.
