...
**Is an instance — null geodesics as light rays.** A null geodesic is a curve $\gamma$ whose tangent $\dot\gamma$ is always null and parallel-transported. In Minkowski space these are straight lines with $\eta(\dot\gamma, \dot\gamma) = 0$ — light rays. In curved spacetime they are the worldlines of light propagating through the gravitational field; gravitational lensing is the deflection of null geodesics by curvature.
...
**Corollary — the trichotomy is invariant under isometries.** If $F : (M, g) \to (N, h)$ is a Lorentzian isometry and $v \in T_pM$, then $h(dF_p v, dF_p v) = g(v, v)$, so $v$ and $dF_p v$ have the same sign of norm-squared, hence the same causal classification. Causal classification is preserved by isometries.
