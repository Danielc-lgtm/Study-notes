...
> **Corollary (Constant-rank level set theorem).** More generally, if $\Phi : M \to N$ has constant rank $r$ in a neighbourhood of every point of $\Phi^{-1}(c)$, then each level set of $\Phi$ is a properly embedded smooth submanifold of codimension $r$ in $M$.
...
Combine the conclusion with **a function to be optimised on the level set.** Property $D$: you want to extremise $f \in C^\infty(M)$ subject to $\Phi = c$. The amplified result $E$: since $\Phi^{-1}(c)$ is a submanifold, an extremum of $f|_{\Phi^{-1}(c)}$ at $p$ has $df_p|_{T_p S} = 0$, i.e., $df_p \in (T_p S)^\perp = \mathrm{span}(d\Phi^1_p, \dots, d\Phi^k_p)$, so there exist Lagrange multipliers $\lambda_i$ with $df_p = \sum \lambda_i d\Phi^i_p$ — the Lagrange multiplier condition. The regular value theorem is what *justifies the geometry* of the [[Thm - The Method of Lagrange Multipliers|Lagrange multiplier]] argument: it guarantees the constraint set has a tangent space at every point.
...
Combine the conclusion with **a group structure on the level set.** Property $D$: $\Phi^{-1}(c)$ is also closed under a group operation (e.g., $\mathrm{SL}(n)$ is closed under matrix multiplication). The amplified result $E$: $\Phi^{-1}(c)$ is a Lie group — a manifold and a group at once, with smooth multiplication and inversion (smoothness is inherited from $M$'s smoothness). Its tangent space at the identity, $\ker d\Phi_e$, is its Lie algebra. This is *exactly* how the matrix Lie groups $\mathrm{SL}(n), \mathrm{O}(n), \mathrm{U}(n), \mathrm{SU}(n), \mathrm{Sp}(2n)$ are recognised as Lie groups: each is a regular level set of a smooth map on $\mathrm{Mat}_n$, and each is closed under multiplication.
...
> When a Lie group $G$ acts freely and properly on a manifold $M$, the orbit space $M/G$ is a smooth manifold and the projection $M \to M/G$ is a surjective submersion. The fibres of this projection are the orbits — each a copy of $G$ (or a coset $G/G_p$). The regular value theorem provides the structure of each orbit; the smooth quotient theorem provides the structure of the orbit space. Homogeneous spaces $G/H$ are the fundamental examples.

> [!tip] The Mass Shell in Quantum Field Theory *(from Physics)*
> The on-shell condition $p^\mu p_\mu = -m^2 c^2$ in [[Special Relativity I — Lorentz Transformations and Minkowski Space|Minkowski space]] defines the mass shell as a regular level set — a $3$-dimensional hyperboloid in $4$-momentum space. Quantum field theory's on-shell scattering amplitudes are defined precisely on this submanifold; off-shell extensions to all of momentum space are propagators integrated against. The submanifold structure of the mass shell is what makes the on-shell / off-shell distinction precise.
