...
- [[Ex - The Orthogonal Group as a Regular Level Set]] (⭐⭐⭐) — Apply the regular value theorem to $\Phi(A) = A^T A$ with codomain $\mathrm{Sym}_n$ (the symmetric matrices, not all matrices — this is the subtle codomain choice). Show $\mathrm{O}(n)$ is an embedded submanifold of dimension $n(n-1)/2$ with $T_I \mathrm{O}(n) = \mathfrak{o}(n)$, the antisymmetric matrices. The codomain-choice trick is the recurring technique for matrix Lie groups defined by an equation with hidden symmetry. ([[Def - Embedded Submanifold]], [[Def - Regular and Critical Points]], [[Def - Tangent Space of a Submanifold]], [[Thm - Regular Value Theorem on Manifolds]])
