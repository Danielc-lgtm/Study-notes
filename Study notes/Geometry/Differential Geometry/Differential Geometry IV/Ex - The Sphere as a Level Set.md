...
is an embedded smooth submanifold of $\mathbb{R}^{n+1}$ of dimension $n$, by applying the [[Thm - Regular Value Theorem on Manifolds|regular value theorem]] to the function $f(x) = |x|^2$. Compute the tangent space $T_p S^n$ explicitly as a subspace of $T_p \mathbb{R}^{n+1} \cong \mathbb{R}^{n+1}$, and verify that it is the orthogonal complement of $p$.
