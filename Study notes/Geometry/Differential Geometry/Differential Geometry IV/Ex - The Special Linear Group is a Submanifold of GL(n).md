...
is an embedded smooth submanifold of $\mathrm{GL}(n,\mathbb{R})$ of dimension $n^2 - 1$. Compute the tangent space at the identity $T_I \mathrm{SL}(n,\mathbb{R})$ explicitly, and verify it equals
...
**The exponential map produces curves through the identity.** The construction $A(t) = e^{tX}$ for $X \in \mathfrak{sl}(n)$ gives a smooth curve in $\mathrm{SL}(n)$ through the identity with velocity $X$. This is the **exponential map** of the Lie group, and it is the standard way of producing tangent vectors at the identity. The general fact: for a matrix Lie group $G$ with Lie algebra $\mathfrak{g} = T_I G$, the exponential map $\exp : \mathfrak{g} \to G$ is a local diffeomorphism near the origin, and its differential at $0$ is the identity on $\mathfrak{g}$. This is developed in [[Differential Geometry XI — Lie Groups, Lie Algebras, and the Exponential Map|DG XI]], but the exercise above gives a first taste.
