...
> A naive attempt is to view $\Phi(A)=A^TA-I$ as a map $\mathrm{Mat}_n\to\mathrm{Mat}_n$. Its values are symmetric, and its differential $d\Phi_A(X)=A^TX+X^TA$ also lies in the **linear** subspace $\mathrm{Sym}_n$. Thus its rank is at most $n(n+1)/2<n^2$, so $0$ is not regular for this unnecessarily large codomain. The repair is to use $\Phi:\mathrm{Mat}_n\to\mathrm{Sym}_n$; there is no “translated tangent space” $\mathrm{Sym}_n-I$ involved, because differentials take values in the tangent space $T_{\Phi(A)}\mathrm{Sym}_n\cong\mathrm{Sym}_n$.
