...
Why must we allow a *flow domain* $\mathcal{D}\subsetneq\mathbb R\times M$? Integral curves can escape in finite time, so their maximal intervals $\mathcal D^{(p)}$ need not be all of $\mathbb R$. Requiring every vertical slice to be an interval containing zero records maximal-curve time domains. Requiring $\mathcal D$ itself to be open is a separate, stronger condition: open vertical slices alone do not imply openness in the product. The fundamental flow theorem obtains product openness from uniform local existence and smooth dependence on initial conditions.

The **[[Def - Group|group]] laws** $\phi(0, p) = p$ and $\phi(t, \phi(s, p)) = \phi(t+s, p)$ deserve a moment's attention. The first is automatic: $\phi^{(p)}(0) = p$ is the starting condition. The second is the substantive content: it says that "follow $X$ for time $s$, then for time $t$" is the same as "follow $X$ for time $t + s$" — which is exactly the autonomous nature of the ODE $\dot\gamma = X(\gamma)$. The proof is the translation lemma for integral curves: the curve $s \mapsto \phi^{(p)}(s + t)$ is the integral curve starting at $\phi^{(p)}(t)$, hence equals $\phi^{(\phi_t(p))}$. So the group law is the geometric statement of "time-translation symmetry of an autonomous ODE".

For a maximal flow the domain identity $\mathcal D^{(\phi_s(p))}=\mathcal D^{(p)}-s$ shows that, once $s\in\mathcal D^{(p)}$, the conditions $t\in\mathcal D^{(\phi_s(p))}$ and $s+t\in\mathcal D^{(p)}$ are equivalent. Stating the group law only where its compositions are defined prevents local-flow notation from concealing a domain assertion.

Why insist on smoothness? Because we want the [[Def - Diffeomorphism|diffeomorphisms]] $\phi_t$ for varying $t$ to form a smooth family, and because in applications we need to differentiate the flow with respect to both arguments (the $\partial_t$ derivative recovers the vector field; the $\partial_p$ derivative gives the linearization of the flow). The smoothness is not for free — it comes from the smooth-dependence-on-initial-data conclusion of [[Thm - The Contraction Mapping Principle|Picard–Lindelöf]], applied via the [[Thm - Fundamental Theorem on Flows|Fundamental Theorem on Flows]].
...
A *global* flow on $M$ is a smooth left action of $(\mathbb R,+)$ on $M$. Equivalently, $t\mapsto\phi_t$ is a one-parameter subgroup of the abstract group $\mathrm{Diff}(M)$ whose evaluation map $(t,p)\mapsto\phi_t(p)$ is smooth. No infinite-dimensional Lie-group structure on $\mathrm{Diff}(M)$ is needed for this equivalence.
...
**Corollary (orbits partition $M$).** The orbit through an equilibrium is a point. A regular orbit carries an immersed one-manifold structure (a line or, for a periodic orbit, a circle), and uniqueness makes two orbits equal or disjoint. If $X$ is nowhere zero these orbits form a regular one-dimensional foliation. In the presence of equilibria their dimensions jump, so the partition is a singular foliation rather than an ordinary foliation.
