...
In dynamics, completeness means that time evolution is globally defined. A conserved Hamiltonian guarantees completeness when the relevant energy surface is compact, because trajectories remain in that compact set; bounded energy by itself is not enough on a noncompact phase space. Geodesic incompleteness is the analogous failure for the geodesic spray and is central in general relativity.
...
> A Riemannian manifold is **geodesically complete** if its geodesics extend for all affine time, equivalently if the geodesic spray is complete. Hopf–Rinow equates this with metric completeness in the Riemannian case. No analogous metric equivalence holds for Lorentzian manifolds; singularity theorems derive causal-geodesic incompleteness from specific curvature, causality, and trapped-set hypotheses.
