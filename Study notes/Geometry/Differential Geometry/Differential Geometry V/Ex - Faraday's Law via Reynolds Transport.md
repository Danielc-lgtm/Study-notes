...
**(a)** Using the [[Thm - Reynolds Transport Theorem|Reynolds Transport Theorem]] applied to the magnetic flux $2$-form $\beta = \star\vec B^\flat$, compute $\frac{d}{dt}\int_{S(t)}\beta$ and decompose it into the explicit-time-dependence term $\int_{S(t)}\partial_t\beta$ plus the Lie-derivative (flow) term $\int_{S(t)}\mathcal{L}_{\vec u}\beta$.
...
> [!note]- Hint 1
> Reynolds's theorem applied to a *time-dependent* form on a moving surface has *two* terms: the explicit time derivative $\partial_t\beta$ and the Lie-derivative-along-the-flow $\mathcal{L}_{\vec u}\beta$. Write both down, and remember that we are integrating a $2$-form over a $2$-surface, so the result has the right [[Def - Dimension|dimensions]] to compare to a flux.
...
> Consider a square loop with oriented normal $+\hat z$ in a uniform field $\vec B=B_0\hat z$, whose right edge of length $L$ moves with $\vec u=u_0\hat x$. Then $d\Phi/dt=B_0Lu_0$. The induced boundary orientation points along $+\hat y$ on that edge, while $\vec u\times\vec B=-u_0B_0\hat y$. With no laboratory electric field,
> $$\mathcal E=\oint_{\partial S}(\vec u\times\vec B)\cdot d\vec\ell=-u_0B_0L=-\frac{d\Phi}{dt}.$$
> The stationary edges contribute nothing. This fixes both signs by one declared orientation; inserting an additional minus sign in the definition of the motional EMF would double-count Faraday's sign.
