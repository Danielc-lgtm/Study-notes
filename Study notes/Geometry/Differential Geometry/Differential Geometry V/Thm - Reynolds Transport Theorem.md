...
> **Theorem (Reynolds Transport — Time-Dependent Form).** With $X = X(t, \cdot)$ a smooth time-dependent vector field, $\phi_t$ the (in general not group-like) family of diffeomorphisms it generates, $D$ compact oriented as above, and $\omega_t \in \Omega^p(M)$ a smooth time-dependent $p$-form,
