...
A **Lie subalgebra** $\mathfrak{h} \subseteq \mathfrak{g}$ is a vector [[Def - Subspace|subspace]] closed under the bracket: $[X, Y] \in \mathfrak{h}$ whenever $X, Y \in \mathfrak{h}$. It is itself a Lie algebra under the restricted bracket.

An **ideal** $\mathfrak{h} \subseteq \mathfrak{g}$ is a vector subspace satisfying the stronger condition $[X, Y] \in \mathfrak{h}$ whenever $X \in \mathfrak{g}$ and $Y \in \mathfrak{h}$ (one of the inputs is from $\mathfrak{h}$). [[Def - Ideal|Ideals]] are normal [[Def - Subgroup|subgroups]]' algebraic shadow — see [[Thm - The Closed Subgroup Theorem]] and Lee Thm 20.28.
