...
as smooth manifolds, with the [[Def - Dimension|dimensions]] agreeing: $\dim S^2 = 2 = \dim \mathrm{SO}(3) - \dim \mathrm{SO}(2) = 3 - 1$.
...
**Problem class:** Realize a familiar manifold ($S^2$) as a homogeneous space $G/H$ for a Lie [[Def - Group|group]] $G$ with closed [[Def - Subgroup|subgroup]] $H$. The route is orbit-stabilizer: verify the action is smooth and transitive, compute the stabilizer.

**Assumption pattern:** $\mathrm{SO}(3)$ is the rotation group of $\mathbb{R}^3$; its action on $S^2$ is the restriction of the linear action on $\mathbb{R}^3$. Two key facts: (1) any rotation can be decomposed as a sequence of two simpler rotations (e.g., Euler angles), making the action transitive; (2) the stabilizer of a single point on the sphere is the [[Def - Subgroup|subgroup]] fixing the line through that point, which is rotations around that axis — a copy of $\mathrm{SO}(2)$.
...
2. **Use the orbit map to embed a homogeneous space (operation 9 from the topic page).** The orbit map $\theta^{(N)} : \mathrm{SO}(3) \to S^2$, $A \mapsto A \cdot N$, descends to a $G$-equivariant [[Def - Diffeomorphism|diffeomorphism]] $\mathrm{SO}(3)/\mathrm{SO}(2) \to S^2$.
...
> [!note]- Hint 4
> Apply orbit-stabilizer: $S^2 \cong \mathrm{SO}(3)/\mathrm{SO}(2)$ as smooth manifolds (via the orbit map descended through the quotient). [[Def - Dimension|Dimensions]] check out: $\dim \mathrm{SO}(3) = 3$, $\dim \mathrm{SO}(2) = 1$, $\dim S^2 = 2 = 3 - 1$.
...
By [[Thm - Orbit-Stabilizer for Lie Group Actions|smooth orbit-stabilizer]] applied to the smooth transitive action of $\mathrm{SO}(3)$ on $S^2$ with stabilizer $G_N \cong \mathrm{SO}(2)$ at the basepoint $N$, the orbit map $\theta^{(N)} : \mathrm{SO}(3) \to S^2$ descends to a $\mathrm{SO}(3)$-equivariant [[Def - Diffeomorphism|diffeomorphism]]
...
**Spheres as homogeneous spaces of orthogonal [[Def - Group|groups]].**
...
**The Hopf [[Def - Fibration|fibration]] is the next level of structure.**

The construction $S^3 = \mathrm{SU}(2)$ acts on $S^2 = \mathrm{SO}(3)/\mathrm{SO}(2)$ via the double cover $\mathrm{SU}(2) \to \mathrm{SO}(3)$, with stabilizer at $N$ being a circle (the preimage of $\mathrm{SO}(2)$ under the double cover is a circle subgroup of $\mathrm{SU}(2)$, which is $S^1 \cong \mathrm{U}(1)$). So $S^2 \cong \mathrm{SU}(2)/\mathrm{U}(1) \cong S^3/S^1$ — the famous **Hopf [[Def - Fibration|fibration]]** $S^1 \to S^3 \to S^2$. This is the first non-trivial example of a non-trivial principal $S^1$-bundle, and it is the simplest case of the **Yang–Mills monopole** in physics. The construction in this exercise is the first step of an extended chain of homogeneous-space identifications leading to fundamental examples in topology and physics.
