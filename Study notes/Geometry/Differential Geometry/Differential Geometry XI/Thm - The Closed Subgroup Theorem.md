...
>
> Moreover, the Lie algebra of $H$, regarded as a [[Def - Subspace|subspace]] of $\mathfrak{g}$, is
...
In practice, the closed subgroup theorem is what underlies the entire theory of matrix Lie [[Def - Group|groups]]. The classical Lie [[Def - Group|groups]] — $\mathrm{O}(n)$, $\mathrm{SO}(n)$, $\mathrm{SL}(n)$, $\mathrm{U}(n)$, $\mathrm{SU}(n)$, $\mathrm{Sp}(2n)$ — are defined as the solution sets of polynomial equations in $\mathrm{GL}(n)$. These solution sets are *closed* by continuity, and they are *[[Def - Subgroup|subgroups]]* by the algebra of the equations. The closed subgroup theorem then says they are automatically embedded Lie [[Def - Subgroup|subgroups]] of $\mathrm{GL}(n)$, with manifold structures and Lie algebras derived from the equations. Without this theorem, each classical group would require its own ad hoc construction of charts and atlases; with it, the construction is uniform.
...
The proof is one of the more sophisticated in Lee's book, and it has several conceptual ingredients. The central observation is that **the structure of $\mathfrak{h}$ can be determined from $H$ before $H$ is even known to be a manifold**: namely, $\mathfrak{h} := \{X \in \mathfrak{g} : \exp(tX) \in H \text{ for all } t \in \mathbb{R}\}$ makes sense without any manifold assumption on $H$, and the proof shows that this $\mathfrak{h}$ is a linear [[Def - Subspace|subspace]] of $\mathfrak{g}$ that turns out to be the Lie algebra of $H$ once the manifold structure is established.
...
>
> **Step 4 (embedded submanifold).** The collection of slice charts $\{\varphi \circ L_{h^{-1}} : h \in H\}$ exhibits $H$ as an embedded submanifold of $G$ of [[Def - Dimension|dimension]] $k = \dim \mathfrak{h}$.
>
...
>
> **Step 6 (Lie algebra equality).** By Step 4, $\dim H = \dim \mathfrak{h}$. By [[Thm - Left-Invariant Vector Fields Form a Lie Algebra]], $\dim \mathrm{Lie}(H) = \dim H = \dim \mathfrak{h}$. Since $\mathrm{Lie}(H) \subseteq \mathfrak{h}$ (a left-invariant vector field on $H$, restricted to $\exp$ of its initial vector, gives an element with $\exp(tX) \in H$ for all $t$), and both have the same finite [[Def - Dimension|dimension]], they are equal: $\mathrm{Lie}(H) = \mathfrak{h}$.
>
