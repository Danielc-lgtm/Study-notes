...
- **Show that $\omega^1, \dots, \omega^k$ are linearly independent iff $\omega^1 \wedge \cdots \wedge \omega^k \neq 0$** (⭐⭐) — uses the determinant identity in both directions: if dependent, expand one as a combination of others and the wedge picks up a repeated factor; if independent, extend to a basis and evaluate ([[Def - Alternating Tensor and Lambda k V Dual]], [[Thm - Wedge Product Properties]])
