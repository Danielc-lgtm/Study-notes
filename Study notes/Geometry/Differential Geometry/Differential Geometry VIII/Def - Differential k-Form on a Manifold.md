...
**Vector-space structure and [[Def - Module|module]] structure.** $\Omega^k(M)$ is an $\mathbb{R}$-vector space (pointwise addition and scalar multiplication of forms). It is also a module over the [[Def - Ring|ring]] $C^\infty(M)$: if $f \in C^\infty(M)$ and $\omega \in \Omega^k(M)$, then $(f\omega)_p = f(p)\,\omega_p$ is a smooth $k$-form, and $\Omega^k(M)$ is closed under these operations.
...
**Corollary — vector-space dimension at a point.** $\dim_{\mathbb{R}} \Lambda^k T_p^*M = \binom{n}{k}$, with maximum at $k = \lfloor n/2 \rfloor$ and the palindrome $\binom{n}{k} = \binom{n}{n-k}$. Total dimension $\sum_k \binom{n}{k} = 2^n$. On $\mathbb{R}^3$ the [[Def - Dimension|dimensions]] are $1, 3, 3, 1$ — the source of the dimension-three coincidence that lets $1$-forms and $2$-forms both be disguised as vector fields.
