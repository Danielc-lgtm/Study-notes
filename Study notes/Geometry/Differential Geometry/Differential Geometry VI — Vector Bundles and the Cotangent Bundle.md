...
- **[[Def - Subbundle]]**
	- A **subbundle** $D \subseteq E$ of a vector bundle $\pi : E \to M$ is an embedded submanifold of $E$ such that each fibre $D_p = D \cap E_p$ is a linear subspace of $E_p$ of constant dimension $m$, and the restricted projection $D \to M$ makes $D$ a vector bundle over $M$. The constant-rank condition is essential: a "family of subspaces" with jumping dimension is not a subbundle. Examples: the span of a nowhere-vanishing vector field is a rank-$1$ subbundle of $TM$; the kernel of a constant-rank bundle homomorphism is a subbundle. Subbundles are the bundle analogue of subspaces of a vector space.
...
> [!tip] Unlocked: Connection on a Vector Bundle *(from Gauge Theory and Riemannian Geometry)*
...
> The differential $dF_p$ exists for any smooth $F : M \to N$, so it is tempting to define a "push-forward" $F_* X$ of a vector field $X$ on $M$ by $(F_* X)_{F(p)} := dF_p(X_p)$. But this is **not well-defined** in general: if two points $p \neq q$ have $F(p) = F(q)$, then $dF_p(X_p)$ and $dF_q(X_q)$ are both candidates for $(F_*X)_{F(p)}$ and there is no reason they should agree. The standard counterexample is $F : \mathbb{R} \to \mathbb{R}$, $F(x) = x^2$, with $X = \partial/\partial x$: at $x = 1$ the push-forward would have to be $2 \partial/\partial y$, at $x = -1$ it would have to be $-2 \partial/\partial y$, but both sit over $y = 1$. Push-forward of vector fields is legal **exactly when $F$ is a diffeomorphism**, where $F^{-1}$ resolves the ambiguity. By contrast, *pullback of covector fields is always legal*, because covectors are evaluated at the target and composed with $dF$.
...
If the problem **involves a smooth map $F : M \to N$ and forms or vectors**, decide which direction $F$ acts in. **Covector fields and forms pull back**: $F^*\omega$ exists for any smooth $F$. **Vector fields push forward only along diffeomorphisms**: $F_*X$ requires $F$ to be a diffeomorphism. The differential $dF_p$ between tangent spaces always exists, but it does not assemble into a pushforward of vector fields unless $F$ is invertible. The reason is structural: a covector at the target only needs a tangent vector at the source to evaluate, so composition with $dF$ does the job; a tangent vector at the source has many possible targets if $F$ is not injective. The route for problems of this type is: identify which objects live where, then transport them in the direction the map allows.
