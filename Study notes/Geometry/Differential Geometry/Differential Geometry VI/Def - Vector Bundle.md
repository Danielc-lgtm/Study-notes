...
**Corollary — sections of a vector bundle always exist.** For any smooth vector bundle $E \to M$, the **zero section** $\sigma_0(p)=0_p\in E_p$ is a smooth global section, so $\Gamma(E)$ is nonempty. This does not imply that $\Gamma(E)$ has positive dimension as a real vector space: for example, the zero-rank bundle has only its zero section. The topologically interesting question is whether a positive-rank bundle admits a *nowhere-vanishing* section.
