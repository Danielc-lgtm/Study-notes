...
What is forced by demanding the trivializations be **diffeomorphisms** rather than just bijections? Without this, the total space $E$ would inherit no smooth structure from the trivializations, and "smooth section" would be undefined. The diffeomorphism condition is the technical content of "smoothly varying": it says the local picture of the bundle is a smooth product, and the smooth structure on $E$ is forced uniquely by this requirement. Drop the diffeomorphism condition (or weaken it to homeomorphism) and you get a **topological vector bundle**, where sections can be continuous but not differentiated.
