...
A subbundle is the bundle counterpart of a vector subspace, made to vary smoothly with $p$. The motivating examples are direct: the kernel of a constant-rank bundle homomorphism, the span of a nonvanishing vector field, the kernel of a 1-form on tangent vectors — all are families of subspaces parametrized by $M$, and all deserve to be regarded as bundles in their own right.
