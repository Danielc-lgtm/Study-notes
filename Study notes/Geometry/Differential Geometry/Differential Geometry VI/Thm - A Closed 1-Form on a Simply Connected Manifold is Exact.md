...
**Conservative forces and potentials.** For a force field on a simply connected domain, $\nabla\times\mathbf F=0$ is the statement that its metric-dual 1-form $\omega$ is closed. The theorem gives $\omega=-dU$ after the conventional sign choice for potential energy. A divergence-free magnetic field corresponds instead to a closed **2-form**; obtaining a vector potential from it requires a higher-degree de Rham or Poincaré result and stronger topology than simple connectivity alone.
