...
This places the Green operator inside the general framework of **Moore–Penrose pseudoinverses** of bounded linear operators with closed range on Hilbert spaces. The novelty of the Hodge-theoretic Green operator is that the operator $\Delta$ being inverted is differential (specifically elliptic), and the Hilbert space $L^2\Omega^k(M)$ has additional structure (it is a graded module over functions, with an algebra structure from the wedge product) that makes the generalized inverse compatible with the other operations of the de Rham complex.
