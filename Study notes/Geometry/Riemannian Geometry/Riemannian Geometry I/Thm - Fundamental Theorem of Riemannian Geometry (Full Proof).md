...
**2. The Levi-Civita connection on a compact bi-invariantly-metric Lie group.** Use the Koszul formula to show that for left-invariant vector fields $X, Y$ on a Lie group $G$ with bi-invariant Riemannian metric, $\nabla_X Y = \tfrac{1}{2}[X, Y]$. Deduce that geodesics through the identity are one-parameter subgroups $t \mapsto \exp(tX)$, and that the Riemannian exponential map equals the Lie-group exponential map of [[Differential Geometry XI — Lie Groups, Lie Algebras, and the Exponential Map|DG XI]]. (Standard problem in Lie-group geometry.)
