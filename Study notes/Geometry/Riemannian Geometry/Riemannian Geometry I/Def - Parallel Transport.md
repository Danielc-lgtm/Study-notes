...
**Example: parallel transport on a Lie group with bi-invariant metric.** On a Lie group $G$ with bi-invariant metric, the Levi-Civita connection has $\nabla_X Y = \tfrac{1}{2}[X, Y]$ on left-invariant fields. Parallel transport along a one-parameter subgroup $\gamma(t) = \exp(tX)$ takes a left-invariant vector $Y$ at the identity to the left-invariant vector $\mathrm{Ad}_{\exp(tX/2)}(Y)$ at $\exp(tX)$. This is the special parallel-transport formula for compact Lie groups and is central to harmonic analysis on Lie groups.
