...
**Why "perfect" rather than "[[Def - Ideal|ideal]]" or "general"?** The qualifier "perfect" means: no anisotropic stress (no shear, no viscosity), no heat conduction (energy flow exactly along $u^\mu$). Real fluids deviate from these idealisations; the perfect-fluid form is the leading-order approximation. Including viscosity adds a term $\eta \sigma^{\mu\nu}$ (with $\sigma$ the shear tensor), and including heat conduction adds a $u^\mu q^\nu + u^\nu q^\mu$ term (with $q^\mu$ the heat flux orthogonal to $u$). For most astrophysical applications, the perfect-fluid form is sufficient.
