...
- **[[Thm - Pi_1 of S^1 is Z|\pi₁(S^1) = ℤ]]**: the prototype non-trivial $\pi_1$, and via Brouwer/no-retraction the gateway to almost every classical topology theorem about the plane. Use it as the *first* example whenever building intuition, and as the *engine* of the Brouwer fixed-point theorem and the fundamental theorem of algebra. The recognition trigger: anything involving rotation, winding, or maps to or from a circle.
