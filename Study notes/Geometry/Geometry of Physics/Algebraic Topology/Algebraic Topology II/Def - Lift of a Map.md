...
> The lift-of-a-path picture generalises to a **connection** on a principal bundle: given a path in the base and a starting point in the fibre, the connection prescribes a unique **horizontal lift**, the **parallel transport** of the starting point along the path. The closed-loop version of this is the **holonomy** of the connection, an analogue of the monodromy action of $\pi_1$ on the fibre. The covering-space case is the special instance where the fibre is discrete and the connection has no continuous freedom.
