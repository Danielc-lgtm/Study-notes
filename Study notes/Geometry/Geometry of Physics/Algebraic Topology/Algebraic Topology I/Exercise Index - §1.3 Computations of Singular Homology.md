...
- [[Ex - Computing the Homology of the Torus]] (⭐⭐) — Compute $H_*(T^2; \mathbb{Z})$ from the polygon triangulation, identifying the generating $1$-cycles and verifying $\partial[T^2] = 0$ from the boundary identifications. Drills simplicial chain-complex computation, fundamental-class construction, and the Künneth-like product structure of $H_*(T^n) = H_*(S^1)^{\otimes n}$. ([[Thm - Singular and Simplicial Homology Agree on Triangulable Spaces]], [[Def - Singular Homology]], [[Def - Euler Characteristic]])
