...
$$
where each $\sigma_i : \Delta^p \to M$ is a [[Def - Singular Simplex|singular p-simplex]] and each $g_i \in G$. Equivalently, a singular $p$-chain is a function $c : \{\text{singular }p\text{-simplices}\} \to G$ that takes the value $0$ except on finitely many simplices.
