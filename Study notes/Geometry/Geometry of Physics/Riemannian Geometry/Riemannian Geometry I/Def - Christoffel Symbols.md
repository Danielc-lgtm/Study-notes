...
**Example: the round 2-sphere.** Metric $g = d\theta^2 + \sin^2\theta\,d\varphi^2$. The Christoffel formula gives $\Gamma^\theta_{\varphi\varphi} = -\sin\theta\cos\theta$, $\Gamma^\varphi_{\theta\varphi} = \Gamma^\varphi_{\varphi\theta} = \cot\theta$, all others zero. The [[Def - Geodesic|geodesic]] equation then reproduces the great-circle equations. See [[Ex - Christoffel Symbols of the Round Metric on the Sphere]].
