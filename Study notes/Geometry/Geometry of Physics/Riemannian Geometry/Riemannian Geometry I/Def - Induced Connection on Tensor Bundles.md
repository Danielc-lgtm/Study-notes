...
> The induced connection on $\mathrm{End}(E)$ (for $E$ a vector bundle with connection) acts on the curvature 2-form $F \in \Omega^2(M; \mathrm{End}\,E)$, and the **Yang-Mills equations** $\nabla\star F = 0$ are a PDE on connections. The variational characterisation: $\nabla\star F = 0$ are the Euler-Lagrange equations of the Yang-Mills action $S_{\mathrm{YM}} = \tfrac{1}{2}\int\mathrm{tr}(F \wedge \star F)$. The full theory, including self-dual and anti-self-dual solutions (instantons), is the content of [[Gauge Theory IV — Yang–Mills Fields and Instantons]].
