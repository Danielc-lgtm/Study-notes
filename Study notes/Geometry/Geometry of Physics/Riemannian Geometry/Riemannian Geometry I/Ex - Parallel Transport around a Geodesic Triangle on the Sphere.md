...
**Theorem routing:** Use the angle-preservation property of parallel transport along [[Def - Geodesic|geodesics]] ([[Thm - Parallel Transport is an Isometry for Metric-Compatible Connections]] applied with $V = W$): the angle of the transported vector relative to the geodesic is preserved. At each vertex, parallel transport from the incoming geodesic's velocity frame to the outgoing geodesic's velocity frame changes the reference direction by the turning angle of the path. The total rotation is the sum of turning angles, which equals $2\pi$ minus the sum of interior angles (Gauss-Bonnet for spherical triangles).
