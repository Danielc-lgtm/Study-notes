...
> The Riemann curvature tensor is the curvature of the Levi-Civita connection on the frame bundle $\mathrm{Fr}(M)$, a principal $O(n)$-bundle. The components $R^a_{\;bcd}$ are exactly the components of the **$\mathfrak{o}(n)$-valued curvature 2-form** $\Omega$ on $\mathrm{Fr}(M)$ pulled back to $M$ via a local frame, and the Bianchi identities are special cases of the universal Bianchi identity $d^A\Omega = 0$ valid for any connection on any principal bundle. See [[Gauge Theory III — Principal Connections, Curvature, Holonomy, and Gauge Symmetry]].
