...
- **To the **Chern–Gauss–Bonnet theorem**.** The higher-dimensional generalisation: for a closed oriented Riemannian $2n$-manifold $M$, $\chi(M) = (1/(2\pi)^n)\int_M\mathrm{Pf}(\Omega)$, where $\Omega$ is the curvature $2$-form of the Levi-Civita connection and $\mathrm{Pf}$ is the Pfaffian (a polynomial in the components of $\Omega$). For $n = 1$, $\mathrm{Pf}(\Omega) = K\, dA$, recovering the surface case. The general theorem was proved by Chern in 1944 using the **transgression form** on the principal $SO(2n)$-bundle, and is the gateway to all of characteristic-class theory.
