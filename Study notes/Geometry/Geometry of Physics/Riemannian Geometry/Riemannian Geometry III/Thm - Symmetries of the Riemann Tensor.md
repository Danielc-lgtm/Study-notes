...
1. **Yang–Mills field strengths.** In Yang–Mills theory with gauge group $G$, the field strength $F = dA + \tfrac{1}{2}[A, A]$ of a connection is a $\mathfrak{g}$-valued $2$-form, satisfying the antisymmetry $F(X, Y) = -F(Y, X)$ (analogous to first-pair antisymmetry of $R$) and the universal **Bianchi identity** $d^A F = 0$. The Riemann tensor's first Bianchi identity is the *algebraic* (zero-form) consequence of this. In the Riemannian case, $R$ is constrained more (by metric compatibility's antisymmetry in the second pair) than a general Yang–Mills $F$. See [[Gauge Theory IV — Yang–Mills Fields and Instantons]].
