...
Show that the geodesics of $(\mathbb{H}^2, g)$ are exactly the **vertical lines** $\{x = x_0\}$ and the **semicircles** $\{(x, y) : (x - c)^2 + y^2 = r^2, y > 0\}$ whose centres lie on the $x$-axis.
...
**Problem class:** Identification of geodesics on a specific Riemannian manifold (the hyperbolic plane). As in [[Ex - Great Circles are the Geodesics of the Sphere]], the approach is one of three: direct ODE integration, symmetry/uniqueness, or projection. The hyperbolic plane has fewer "intrinsic" symmetries available as visible reflections than the sphere does, so we use a combined approach: compute Christoffel symbols, use the *isometry group* (Möbius transformations) to reduce to a single case, and integrate the ODE in that case.
