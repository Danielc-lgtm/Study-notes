...
The structural payoff is that *every* property of $\exp_p$ is a property of the geodesic flow: smoothness of $\exp_p$ in $p$ (as well as in $v$) follows from smoothness of $\phi$, equivariance of $\exp_p$ under [[Def - Isometry|isometries]] follows from equivariance of $G$ under isometries, and the Hamiltonian-mechanical interpretation in [[Def - Hamiltonian Flow of the Kinetic Energy]] becomes available because $\phi$ is also (after Legendre transform) the Hamiltonian flow of $H = \tfrac12 |p|^2_{g^{-1}}$ on $T^*M$.
