...
**What if we strengthen — demand $\delta$ commute with [[Def - Isometry|isometries]]?** It does, by construction. The Hodge star commutes with orientation-preserving isometries; the exterior derivative commutes with all smooth maps via pullback. So $\delta = \pm\star d\star$ commutes with orientation-preserving isometries, which is exactly the equivariance we want.
