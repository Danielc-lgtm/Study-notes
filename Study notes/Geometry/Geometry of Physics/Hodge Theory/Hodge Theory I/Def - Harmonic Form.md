...
**What if we drop the Riemannian hypothesis?** Without a metric, $\delta$ does not exist, so $\Delta$ does not exist either, and "harmonic form" has no meaning. Harmonic forms are a *metric-dependent* concept — the same de Rham cohomology class can have different harmonic representatives for different metrics. The cohomology [[Def - Dimension|dimension]] $b_k(M)$ is metric-independent (topological), but the representatives are metric-dependent (geometric). This is the structural reason Hodge theory is a metric enrichment of de Rham theory.
