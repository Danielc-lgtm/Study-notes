...
The case $G = U(1) = \{z \in \mathbb{C} : |z| = 1\}$ already shows the non-trivial content. Here $T_1U(1) = i\mathbb{R}$, the left translation $L_z(w) = zw$ is the restriction of the complex-linear map $w \mapsto zw$, so $d_1L_z(i\theta) = i\theta z$, and the left-invariant field generated by $X = i\theta$ is $X^L_z = i\theta z$ (the tangent vector to the circle at $z$ of length $|\theta|$, pointing anticlockwise if $\theta > 0$). The curve $\gamma(t) = e^{i\theta t}$ satisfies $\gamma'(t) = i\theta e^{i\theta t} = X^L_{\gamma(t)}$, so it is the integral curve of $X^L$ from $1$, and it is a homomorphism because $e^{i\theta(s+t)} = e^{i\theta s}e^{i\theta t}$. Hence $\exp(i\theta) = e^{i\theta}$. The theorem's bijection is between one-parameter subgroups and $\mathfrak{g}$, not between $G$ and $\mathfrak{g}$: the map $\exp$ itself is $2\pi$-periodic on $i\mathbb{R}$, hence not injective, so it is not a bijection onto $G$. One must be careful about *why* $\exp$ fails to be a bijection here, because $U(1)$ is abelian: in fact $\exp : i\mathbb{R} \to U(1)$, $i\theta \mapsto e^{i\theta}$, *is* a group homomorphism from $(\mathfrak{g}, +)$, with kernel $2\pi i\mathbb{Z}$, so for $U(1)$ the obstruction to a bijection with $G$ is only the non-injectivity, not any failure of the homomorphism property. That $\exp$ is a homomorphism on all of $\mathfrak{g}$ is special to abelian groups; in a non-abelian group $\exp$ is a homomorphism only along each individual line $\{tX : t \in \mathbb{R}\}$ — which is exactly what part (d) says — and $\exp(X)\exp(Y) \neq \exp(X + Y)$ as soon as $[X, Y] \neq 0$.
...
> >
> > **Compute $\alpha'(0)$:** by Lemma 2 applied to $\gamma$ with scaling factor $1$ and shift $t$ (so that Lemma 2's reparametrised curve is $s \mapsto \gamma(s + t) = \alpha(s)$), $\alpha'(0) = \gamma'(0 + t) = \gamma'(t)$.
> >
> > **Compute $\beta'(0)$:**
...
> > coincide.
> >
> > **$\alpha$ is an integral curve of $X^L$ with $\alpha(0) = \gamma(t)$.** By Lemma 2 (with scaling factor $1$ and shift $t$; the domain is $\{s : s + t \in \mathbb{R}\} = \mathbb{R}$), $\alpha$ is an integral curve of $X^L$ on $\mathbb{R}$, and $\alpha(0) = \gamma(t)$ by definition.
> >
