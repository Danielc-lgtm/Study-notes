...
> Let $G$ be a compact connected Lie group with Lie algebra $\mathfrak{g}$, and let $g\in G$ be an arbitrary target element. We must produce $X\in\mathfrak{g}$ with $\exp(X)=g$. If $g = e$, then $X = 0$ already works, since $\exp(0) = \gamma_0(1) = e$ (the one-parameter subgroup generated by $0\in\mathfrak{g}$ is the constant curve at $e$); so for the remainder we assume $g\neq e$.
>
...
>
> **Step 2 — a minimizing geodesic from $e$ to the target.** By **Lemma 4**, applied to the compact connected Riemannian manifold $(G,g^{\mathrm{met}})$ and the two points $e$ and $g$, there is a unit-speed geodesic $\sigma\colon[0,\ell]\to G$ with $\sigma(0)=e$, $\sigma(\ell)=g$, and length $\ell=d(e,g)$. Here $\ell = d(e,g) > 0$: the Riemannian distance is a genuine metric inducing the manifold topology, so $d(e,g) = 0$ would force $g = e$, which we excluded. In particular the unit-speed parametrisation and the unit initial vector below are well defined. (Lemma 3 guarantees this geodesic is defined for all the parameter values used, so that "reaching $g$ at parameter $\ell$" is meaningful.) Its initial velocity $\dot\sigma(0)=:v$ lies in $T_eG=\mathfrak{g}$ and is a unit vector, $\lVert v\rVert=1$.
>
