...
**High-level strategy:** First establish the matrix exponential as an analytic object on the Banach algebra $\operatorname{Mat}(n \times n; \mathbb{K})$ — convergence, the functional equation, the derivative — using only submultiplicativity of the operator norm together with three theorems on series whose hypotheses we verify at each use: that an absolutely convergent series in a Banach space converges, the Cauchy-product theorem in a Banach algebra, and the Weierstrass M-test with the term-by-term differentiation theorem for a uniformly convergent series of derivatives. This makes $t \mapsto e^{tX}$ a smooth one-parameter subgroup of $GL(n; \mathbb{K})$ with velocity $X$, so uniqueness of one-parameter subgroups identifies it with $\exp_{GL}$. Then, for a closed subgroup $G$, invoke the closed subgroup theorem to keep the curve inside $G$, and run the uniqueness argument again inside $G$.
