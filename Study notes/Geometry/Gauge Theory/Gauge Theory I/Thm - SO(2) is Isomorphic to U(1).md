...
> **The mechanism in one sentence:** the three orthogonality equations $a^2+b^2 = 1$, $c^2+d^2=1$, $ac+bd=0$ coming from $A^t A = 1$, together with $\det A = 1$, force $c=-b$ and $d=a$ and so collapse the four real entries of a $2\times 2$ matrix down to a single pair $(a,b)$ with $a^2+b^2=1$ — one angle — exactly the datum that also parametrises the unit modulus $|z|=1$, and both groups turn that datum into their product by adding it modulo $2\pi$.
...
> >
> > **Uniqueness up to $2\pi$.** If $R(\varphi) = R(\varphi')$ then in particular $\cos\varphi = \cos\varphi'$ and $\sin\varphi = \sin\varphi'$. Two reals with equal sine and equal cosine differ by a multiple of $2\pi$: the difference $\delta = \varphi - \varphi'$ satisfies $\cos\delta = \cos\varphi\cos\varphi' + \sin\varphi\sin\varphi' = \cos^2\varphi' + \sin^2\varphi' = 1$ (by the cosine subtraction formula and $\cos\varphi=\cos\varphi'$, $\sin\varphi=\sin\varphi'$), and $\cos\delta = 1$ holds if and only if $\delta\in 2\pi\mathbb{Z}$ (the cosine attains its maximum value $1$ exactly at the integer multiples of $2\pi$ and nowhere else, since $\cos\delta = 1 - 2\sin^2(\delta/2)$ equals $1$ if and only if $\sin(\delta/2) = 0$, that is $\delta/2\in\pi\mathbb{Z}$). Conversely, if $\varphi - \varphi'\in 2\pi\mathbb{Z}$ then $\cos$ and $\sin$, being $2\pi$-periodic, agree at $\varphi$ and $\varphi'$, so $R(\varphi) = R(\varphi')$. Therefore $R(\varphi) = R(\varphi')$ if and only if $\varphi\equiv\varphi'\pmod{2\pi}$. $\blacksquare$
...
> >
> > **Uniqueness up to $2\pi$.** Suppose $e^{i\varphi} = e^{i\varphi'}$. Equating real and imaginary parts of $\cos\varphi + i\sin\varphi = \cos\varphi' + i\sin\varphi'$ gives $\cos\varphi = \cos\varphi'$ and $\sin\varphi = \sin\varphi'$. Set $\delta := \varphi - \varphi'$; by the cosine subtraction formula, $\cos\delta = \cos\varphi\cos\varphi' + \sin\varphi\sin\varphi' = \cos^2\varphi' + \sin^2\varphi' = 1$ (using $\cos\varphi=\cos\varphi'$ and $\sin\varphi=\sin\varphi'$), and $\cos\delta = 1$ holds if and only if $\delta\in 2\pi\mathbb{Z}$ (as shown in Lemma 1, since $\cos\delta = 1 - 2\sin^2(\delta/2)$ equals $1$ exactly when $\delta/2\in\pi\mathbb{Z}$). Conversely, if $\varphi - \varphi'\in 2\pi\mathbb{Z}$ then the $2\pi$-periodicity of $\cos$ and $\sin$ gives $\cos\varphi = \cos\varphi'$ and $\sin\varphi = \sin\varphi'$, hence $e^{i\varphi} = e^{i\varphi'}$. Therefore $e^{i\varphi} = e^{i\varphi'}$ if and only if $\varphi - \varphi'\in 2\pi\mathbb{Z}$. $\blacksquare$
