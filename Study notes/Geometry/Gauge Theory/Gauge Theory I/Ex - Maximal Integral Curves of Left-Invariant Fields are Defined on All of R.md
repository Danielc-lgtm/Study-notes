...
>
> *Completeness at $e$.* Let $\gamma\colon(a,b)\to G$ be maximal with $\gamma(0)=e$. If $b<\infty$, pick $s\in(b-\varepsilon,b)$; the curve $\eta(t):=(L_{\gamma(s)}\circ\theta)(t-s)$ (Lemmas A, B) is an integral curve on $(s-\varepsilon,s+\varepsilon)$ with $\eta(s)=\gamma(s)$. By Lemma C, $\gamma$ and $\eta$ glue to an integral curve on $(a,b)\cup(s-\varepsilon,s+\varepsilon)=(a,s+\varepsilon)$ with $s+\varepsilon>b$, contradicting maximality. Hence $b=\infty$; symmetrically $a=-\infty$. So $\gamma\colon\mathbb{R}\to G$.
>
