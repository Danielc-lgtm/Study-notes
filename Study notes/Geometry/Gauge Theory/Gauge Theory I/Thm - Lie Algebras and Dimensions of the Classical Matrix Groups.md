...
> **Statement:** Let $G \subseteq GL(n; \mathbb{K})$ be a matrix Lie group, let $U$ be an open subset of a finite-dimensional real vector space $V$ (either $\operatorname{Mat}(n\times n;\mathbb{K})$ itself, or a submanifold thereof) containing $G$, let $W$ be a finite-dimensional real vector space, and let $\Phi : U \to W$ be smooth with $G = \Phi^{-1}(y_0)$ for some $y_0 \in W$. If $d\Phi_A : V \to W$ is surjective for every $A \in G$, then $G$ is a smooth submanifold with $\dim G = \dim V - \dim W$, and, under the evaluation isomorphism $\mathfrak{g} \cong T_{1_n} G$, the Lie algebra is $\mathfrak{g} = \ker d\Phi_{1_n}$, of dimension $\dim V - \dim W$.
>
...
>
> > [!note]- Full proof
> > **Invoke the regular value theorem.** We use the following, proved in full on [[Thm - Regular Value Theorem on Manifolds]]: *if $\Phi : M \to N$ is smooth and $c \in N$ is a regular value (that is, $d\Phi_p$ is surjective for every $p \in \Phi^{-1}(c)$), then the nonempty level set $\Phi^{-1}(c)$ is a properly embedded submanifold of codimension $\dim N$, and $T_p\big(\Phi^{-1}(c)\big) = \ker d\Phi_p$ for every $p$ in it.* Here $M = U$ is an open subset of $V$, hence a manifold of dimension $\dim V$, and $N = W$ has dimension $\dim W$.
> >
> > **Apply it to $\Phi$ at $y_0$.** By hypothesis $d\Phi_A$ is surjective for every $A \in G = \Phi^{-1}(y_0)$, so $y_0$ is a regular value of $\Phi$. The regular value theorem gives that $G$ is a submanifold of codimension $\dim W$, hence
> > $$\dim G = \dim U - \dim W = \dim V - \dim W \qquad \text{(codimension } = \dim W\text{, and } \dim U = \dim V\text{)},$$
> > and, taking $p = 1_n \in G$,
> > $$T_{1_n} G = \ker d\Phi_{1_n} = \{X \in V : d\Phi_{1_n}(X) = 0\} \qquad \text{(tangent space of a regular level set)}.$$
> >
> > **Identify the Lie algebra.** By [[Thm - Left-Invariant Vector Fields Form a Lie Algebra]] — restated: *for a Lie group $G$ the evaluation map $\varepsilon : \mathfrak{g} \to T_{e} G$, $X \mapsto X_e$, is a linear isomorphism, so $\dim_{\mathbb{R}} \mathfrak{g} = \dim G$* — the Lie algebra $\mathfrak{g}$ is linearly isomorphic to $T_{1_n} G$ (with $e = 1_n$). Under this isomorphism $\mathfrak{g} = T_{1_n} G = \ker d\Phi_{1_n}$, of real dimension $\dim V - \dim W$. Since the kernel of a surjective linear map $V \to W$ has dimension $\dim V - \dim W$ by rank–nullity, the two dimension computations agree, as they must. $\blacksquare$
...
> $$\dim O(n) = \dim \operatorname{Mat}(n \times n; \mathbb{R}) - \dim \operatorname{Sym}(n) = n^2 - \tfrac{n(n+1)}{2} = \tfrac{n(n-1)}{2},$$
> and
...
> $$\dim U(n) = \dim_{\mathbb{R}}\operatorname{Mat}(n \times n; \mathbb{C}) - \dim_{\mathbb{R}}\operatorname{Herm}(n) = 2n^2 - n^2 = n^2,$$
> and
...
> $$\dim_{\mathbb{R}} SL(n; \mathbb{C}) = 2n^2 - 2, \qquad \mathfrak{sl}(n; \mathbb{C}) = \ker d(\det)_{1_n} = \{X \in \operatorname{Mat}(n \times n; \mathbb{C}) : \operatorname{tr} X = 0\}.$$
...
> $$\dim SU(n) = \dim U(n) - \dim U(1) = n^2 - 1,$$
> and
