...
> Indeed, $g_\varphi\, g_\varphi^{-1} = \operatorname{diag}(e^{i\varphi}e^{-i\varphi},\, e^{-i\varphi}e^{i\varphi}) = \operatorname{diag}(1, 1) = 1_2$, confirming the inverse.
...
> The matrix $\left(\begin{smallmatrix} \cos 2\varphi & -\sin 2\varphi & 0 \\ \sin 2\varphi & \cos 2\varphi & 0 \\ 0 & 0 & 1\end{smallmatrix}\right)$ has determinant $\cos^2 2\varphi + \sin^2 2\varphi = 1$ and its columns are orthonormal, so it lies in $SO(3)$. This is the expected outcome: the adjoint action preserves the $\operatorname{Ad}$-invariant inner product $\langle X, Y\rangle = -\tfrac{1}{2}\operatorname{tr}(XY)$ on $\mathfrak{su}(2)$ (a positive multiple of the negative of the Killing form; it is $\operatorname{Ad}$-invariant because $\operatorname{tr}(gXg^{-1}\,gYg^{-1}) = \operatorname{tr}(XY)$ by cyclicity of the trace). In the basis $E_1, E_2, E_3$ this form is orthonormal: each $E_a$ satisfies $E_a^2 = -1_2$, so $\langle E_a, E_a\rangle = -\tfrac{1}{2}\operatorname{tr}(-1_2) = 1$, while for $a \neq b$ the product $E_a E_b$ is, up to sign, one of the three traceless basis matrices (for instance $E_1 E_2 = \left(\begin{smallmatrix} 0 & 1 \\ -1 & 0\end{smallmatrix}\right)\left(\begin{smallmatrix} 0 & i \\ i & 0\end{smallmatrix}\right) = \left(\begin{smallmatrix} i & 0 \\ 0 & -i\end{smallmatrix}\right) = E_3$; likewise $E_2 E_3 = E_1$ and $E_3 E_1 = E_2$), each of trace $0$, giving $\langle E_a, E_b\rangle = 0 = \delta_{ab}$. Hence $\operatorname{Ad}_g$ is an orthogonal transformation of the Euclidean space $(\mathfrak{su}(2), \langle\cdot,\cdot\rangle) \cong \mathbb{R}^3$. That every $\operatorname{Ad}_g$ lands in $SO(3)$ (not merely $O(3)$) is the statement that $\operatorname{Ad} : SU(2) \to SO(3)$ is the double cover.
