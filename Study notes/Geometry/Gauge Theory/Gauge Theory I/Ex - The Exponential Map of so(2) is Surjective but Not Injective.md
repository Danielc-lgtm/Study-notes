...
> so $\exp$ is **not injective**. Under the linear isomorphism $\mathbb{R}\xrightarrow{\sim}\mathfrak{so}(2)$, $\theta\mapsto A_\theta$, the fibre $\{A_\theta:\exp(A_\theta)=\mathbf{1}_2\}$ corresponds precisely to $2\pi\mathbb{Z}\subset\mathbb{R}$. More generally, since $\mathfrak{so}(2)$ is abelian and $A_\theta,A_{\theta'}$ commute, $\exp(A_\theta)\exp(A_{\theta'})=\exp(A_{\theta+\theta'})$ (the matrix exponential is multiplicative on commuting matrices), so $\exp$ is a group homomorphism $(\mathfrak{so}(2),+)\to SO(2)$ and $R_\theta=R_{\theta'}\iff\theta-\theta'\in2\pi\mathbb{Z}$; the fibres of $\exp$ are exactly the cosets of the kernel $2\pi\mathbb{Z}$. This is the sense in which $SO(2)\cong\mathbb{R}/2\pi\mathbb{Z}\cong S^1$ and the exponential is the covering $\mathbb{R}\to S^1$. This proves part 3. $\blacksquare$
