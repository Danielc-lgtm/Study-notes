...
> Therefore $\mathbb{C}\omega$ is a one-dimensional invariant subspace on which every group element acts as the identity: it is a copy of the trivial representation $\varrho_0$. (Here $\det \varrho_1(g) = \det g$ because $\varrho_1$ is the inclusion $g \mapsto g$, and $\det g = 1$ because every element of $SU(2)$ has determinant one by definition.)
>
