...
> Hence the stabiliser of $N$ is the whole group $SO(2) \neq \{e\}$. A single point with a non-trivial stabiliser refutes freeness (by the definition on **[[Def - Free, Transitive, Effective, and Proper Group Actions|the free/effective/transitive definition]]**, a free action would force any $g$ with a fixed point to equal $e$). So the action is **not free**. The same computation applies to the south pole $S = (0, 0, -1)$: its first two coordinates are zero and its third is fixed, so $g \cdot S = \hat{g} S = S$ for every $g \in SO(2)$, and $S$ too is a fixed point of the whole group.
>
...
> where the exponential is computed from $\exp(tJ) = \begin{pmatrix} \cos t & -\sin t \\ \sin t & \cos t \end{pmatrix}$ placed in the upper-left block, the last row and column contributing $\exp(0) = 1$. The planar exponential is itself immediate from $J^2 = \begin{pmatrix} 0 & -1 \\ 1 & 0 \end{pmatrix}^2 = -1_2$: summing the exponential series and separating even from odd powers gives $\exp(tJ) = \sum_{k \geq 0} \frac{t^{2k}}{(2k)!} J^{2k} + \sum_{k \geq 0} \frac{t^{2k+1}}{(2k+1)!} J^{2k+1} = \left(\sum_{k \geq 0} \frac{(-1)^k t^{2k}}{(2k)!}\right) 1_2 + \left(\sum_{k \geq 0} \frac{(-1)^k t^{2k+1}}{(2k+1)!}\right) J = \cos t \, 1_2 + \sin t \, J$ (using $J^{2k} = (-1)^k 1_2$ and $J^{2k+1} = (-1)^k J$), which is exactly the displayed rotation matrix. This one-parameter subgroup $\exp(t\hat{X})$ is exactly the rotation of angle $t$ about the $z$-axis, so its action on $S^2$ is $\theta_{\exp(tX)}(x) = \exp(t\hat{X}) x$.
>
