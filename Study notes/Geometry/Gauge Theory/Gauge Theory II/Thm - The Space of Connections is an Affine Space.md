...
>
> **Step 2 — the action is free (uniqueness of the difference).** Suppose $\nabla + a = \nabla + a'$ for some $\nabla \in \mathcal{A}(E)$ and $a, a' \in V$. Then $\nabla s + a \cdot s = \nabla s + a' \cdot s$, hence $a \cdot s = a' \cdot s$ for every $s \in \Gamma(E)$, so the two bundle-valued $1$-forms $a$ and $a'$ induce **the same** $C^\infty(M)$-linear map $s \mapsto a \cdot s = a' \cdot s : \Gamma(E) \to \Omega^1(M; E)$. By the uniqueness clause of [[Thm - Tensoriality Lemma for C-Infinity-Linear Maps|the tensoriality lemma]] restated in Part (b) — *the bundle-valued form $a \in \Omega^1(M; \operatorname{End} E)$ representing a given $C^\infty(M)$-linear map $\Gamma(E) \to \Omega^1(M; E)$ via $s \mapsto a \cdot s$ is unique* — it follows that $a = a'$.
>
> **Concretely** (to see the uniqueness clause explicitly), a bundle-valued $1$-form $b := a - a' \in \Omega^1(M; \operatorname{End} E)$ with $b \cdot s = 0$ for all $s \in \Gamma(E)$ vanishes: fix $m \in M$ and $v \in E_m$; choose a [[Def - Local Frame|local frame]] $(e_1, \dots, e_k)$ over a trivialising neighbourhood $U \ni m$, write $v = \sum_{j} v^j e_j(m)$ with $v^j \in \mathbb{R}$, and let $\chi \in C^\infty(M)$ be a [[Thm - Existence of Smooth Bump Functions|bump function]] — *for any point $m$ and neighbourhood $U$ there is $\chi \in C^\infty(M)$ with $\chi(m) = 1$ and $\operatorname{supp}\chi \subset U$* — so that $s := \chi \sum_j v^j e_j$, extended by zero outside $U$, is a global smooth section with $s(m) = v$. For every tangent vector $X$ at $m$, $\big(b(X)\big)v = \big(b(X)\big)\big(s(m)\big) = (b \cdot s)(X)\big|_m = 0$; as $v \in E_m$ and $X$ were arbitrary, the endomorphism $b(X)$ is zero for every $X$, so $b$ vanishes at $m$, and as $m$ was arbitrary, $b = 0$, i.e. $a = a'$. Hence for each ordered pair $(\nabla, \hat\nabla)$ the $a$ with $\hat\nabla = \nabla + a$ is unique, so the action is free.
>
