...
**Criterion 2 — Full rigorous solution in a toggle, immediately after the problem.** Directly after the problem description (and its Recall section) place a single collapsible callout containing a complete, rigorous, self-contained solution. Use the repo's collapsed-by-default callout syntax `> [!note]- Full solution` (the trailing `-` collapses it). Inside, follow the repo's established solution structure: a plan paragraph (~3 sentences summarising the whole route), per-step bolded summary lines, a nested `> [!note]- Derivation` callout under each step with the full computation, and a final `> [!note]- Complete formal solution` callout giving a clean self-contained proof. The solution must be **genuinely rigorous** at the vault's proof floor — `../polymath-notes/references/prose-and-proof-standard.md` Part II: named goal, labelled blocks, a justification on every line, every case, a closing sentence — with no skipped steps, and **every invoked theorem wikilinked to a page whose Formal Proof is complete** (a solution that leans on a statement-only or sketched page inherits the violation; write the missing proof or choose a proved route). Progressive `> [!note]- Hint N` callouts may still appear between the problem and the full-solution toggle, but the full rigorous solution toggle is the required centrepiece of this mode.
...
**Prose register and proof floor (all modes).** All explanatory text — problem framings, "why does it work" notes, technique triggers, takeaways, theory toggles — is written in the vault's prose register, which is the owner's thesis (`prose/Chiang Sung En-Thesis.pdf`, specified in `../polymath-notes/references/prose-and-proof-standard.md` Part I; a copy of that file is symlinked into this skill's `references/`): orient, motivate, state, unpack in the smallest concrete case, close in words, in a measured academic voice with every claim carrying its reason. Every solution, derivation, and model solution meets the vault's proof floor (Part II of the same file): every displayed line justified, every case, every invoked result wikilinked to a page that proves it in full. A theory toggle (Mode 4, Mode 5) that *states* a theorem the exercise then uses must either prove it in the toggle at that floor or wikilink a vault page whose Formal Proof is complete; a stated-but-unproved theorem is a violation of the vault's Proof Standard, not a permitted shortcut.
