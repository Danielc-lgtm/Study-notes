...
The notes have two registers. **Formal definitions and theorem statements** should be precise, complete, and stated in standard mathematical language. Everything else — motivations, axiom motivations, "why is it true" explanations, legal operations, bridges, takeaways, problem-solving strategies — should be written in the voice of the two prose exemplars in the `prose/` folder: the user's master's thesis (`prose/Chiang Sung En-Thesis.pdf`) and Axler's *Linear Algebra Done Right* (`prose/linear_algebra_done_right.pdf`). **Read both before writing** (extract with `pymupdf` — `pip install pymupdf`, then `import pymupdf` — or `pdftotext -layout`); the prose voice and the proof-detail floor are the deliverable's defining qualities, and it is easy to drift back into a terse specification register. Both are conversational but precise, clean but never dry, and always ground abstract machinery in the concrete problems it solves.

The exemplars have specific qualities worth emulating:

They write as if talking to you at a whiteboard. Each paragraph advances the reader's understanding, and the text flows naturally from problem to concept to definition, rather than presenting labeled sections with disconnected content. When introducing a concept, they open with the situation that calls for it — what you are trying to do, what goes wrong without the concept, and why this specific construction resolves the difficulty. *Linear Algebra Done Right* opens a section on subspaces with "By considering subspaces, we can greatly expand our examples of vector spaces"; the formal definition arrives as the punchline, not the starting point.

They mix short, direct sentences with longer explanatory ones, and reach for a concrete analogy the moment it clarifies — "Sums of subspaces in the theory of vector spaces are analogous to unions of subsets in set theory." This is the rhythm to aim for. They are honest about difficulty and about what is genuinely deep versus what is merely technical.

They weave in the bigger picture alongside the details, and — this is the non-negotiable part of the exemplars — every result they state is *proved in full*: each proof step names the exact definition, hypothesis, or earlier result it uses ("$-u$ [which equals $(-1)u$ by 1.32] is also in $U$ by the third condition", "1.34 implies …"), so a reader checks each line without supplying anything of their own. The notes should do both: a motivation that illuminates the grand structure, followed by the precise, gap-free computation that makes it rigorous.
...
- Write all other content in the exemplar prose voice (see Writing Style above).
...
2. **Motivation** — why this topic exists, what problems it solves. Written in the exemplar-voice flowing prose (multiple paragraphs; see Writing Style above). Required elements:
...
6. **Examples / Corollaries** — concrete examples and non-examples, and immediate corollaries that serve as calibration checks: if the reader can verify each one, they have understood the definition correctly. Include both "is an instance" and "is NOT an instance" examples — at least one "is NOT" for any non-trivial definition. Each example should probe a different aspect of the definition. End the section with an explicit **`**Calibration check.**`** paragraph naming 2–3 small verifications the reader should be able to perform if they have understood the definition.
...
9. **Formal Proof** — **required, and mandatory on every theorem subpage: a complete, self-contained, rigorous, gap-free proof** in a collapsible section. This is not optional and not a sketch. The proof is written in the voice and to the standard of the two exemplars in `prose/` (see Writing Style): the thesis's labelled-step style — each step a single justified move, nothing left to "clearly", "it follows", "one checks", or a bare "by [X]" — with the *Linear Algebra Done Right* discipline that every step names the exact definition, hypothesis, or earlier result it uses. **The thesis's level of proof detail is the MINIMUM baseline — a proof may be more detailed, never terser.** A reader must be able to check every line on paper, without supplying a step of their own or opening another page. If the full proof runs long, it runs long — length is never a reason to compress it below this floor (see the "Prose over compression" principle in Writing Style). When the theorem has well-posedness preconditions (e.g. "the quotient exists", "the integral converges"), open the proof with **"Step 0 — [precondition]"** verifying it, separately from the substantive proof — see `Thm - First Isomorphism Theorem.md` for the pattern. When you fill a step the source elided or supply reasoning from your own knowledge, verify it and be honest about any residual uncertainty.
...
10a. **Every theorem subpage carries a complete, self-contained, rigorous proof.** Pick any `Thm -`/`Lemma -` subpage: its Formal Proof section is a full gap-free proof, at least as detailed as a proof in the reference thesis (the MINIMUM baseline), with every step naming the definition/hypothesis/earlier result it uses and nothing left to "clearly"/"it follows"/bare "by [X]". A missing proof, a sketch standing in for a proof, or a proof a floor reader cannot check line by line is a quality failure — rewrite it.
...
**Report:** After checking, briefly report which items passed and any that required fixes. If all items pass, state "Self-evaluation passed: all 37 checklist items verified plus cross-subject parity check."
