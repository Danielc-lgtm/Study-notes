...
1. **The reference thesis — the prose register and the minimum proof standard for every page.** `prose/Chiang Sung En-Thesis.pdf` (extract with `pymupdf` — `pip install pymupdf` then `import pymupdf` — or `pdftotext -layout` if poppler is present). **Study its prose closely before writing anything.** The thesis is the prose baseline for the *whole vault*, not only for paper notes: `references/prose-and-proof-standard.md` (a symlink to the polymath-notes copy) is the full specification of the register (Part I, with calibration passages by page number) and of the proof floor (Part II). What you replicate is summarised in **The Prose Standard** below — read that section against the thesis open beside you.
2. **`paper_source/example.md` — the structural exemplar for the Whole-Paper Story page.** A worked example of story-mode writing, covering §3 of the reference paper end to end. It sets the *structure* of the mandatory Whole-Paper Story page (kind 2 in **What you produce**): one flowing narrative with a mental picture for every object, explicit connective sentences at every section boundary, symbols only where they carry weight, and a final one-paragraph total-picture. Its *sentences*, however, are written in the thesis register of item 1 — measured first-person-plural academic prose with every claim carrying its reason — not in the exemplar's chattier voice. Study it before writing the Story page for any new paper; the structural standard is not negotiable.
3. `references/prose-and-proof-standard.md` — the vault-wide prose register and proof standard (every theorem mentioned is proved in full; the thesis's fully written proofs are the floor; the single imported-result exception). Rule 5 below inherits it.
4. `references/notation-discipline.md` — the typing, terminology, and prose-over-compression rules (Rules 3, 4, 7 below live here in full; the body only summarises them).
5. `references/companion-page-template.md` — the index + section-page skeleton and the Whole-Paper Story skeleton.
6. `references/recall-callouts.md` — the point-of-use recall callout, the external-input callout, and the uncertainty marker.
7. `references/atomic-note-templates.md` — the atomic-subpage templates (paper items and load-bearing paragraphs).
8. `references/obsidian-patterns.md` (a symlink to the polymath-notes copy — **the source of truth for Obsidian syntax**: wikilinks, transclusion, collapsible callouts, math delimiters `$...$` / `$$...$$`, never LaTeX inside `[[ ]]`, Windows-portable filenames, YAML frontmatter). When this skill and that file disagree on *syntax*, that file wins — **with two deliberate paper-notes carve-outs**: (a) callout *titles* (fold lines) use Unicode, not LaTeX, for portability across Obsidian versions and themes, even though `obsidian-patterns.md` says markdown renders in titles (LaTeX in a callout *body* is fine — the carve-out is titles only); and (b) the custom callout labels `[!recall]` (a paper-notes device) and `[!cite]` (a native `quote` alias) are intended and permitted, not to be "corrected" against that file's callout-type list. See `references/recall-callouts.md`.
9. `../polymath-notes/SKILL.md` — **Core Philosophy, Writing Style, and The Proof Standard** (self-containedness via DAG links, hierarchical structure, connections, insight density; the thesis register; every theorem mentioned proved). Its Writing Style section now specifies the same thesis register this skill uses, so the two agree. Reuse its atomic note types and its Obsidian conventions. **Do not** run its `autolinker.py` over a paper folder (it mislinks paper-local notation).
...
**The reference thesis (`prose/Chiang Sung En-Thesis.pdf`) is the voice every paper note must be written in — and, since it is the baseline for the whole vault, every other note too.** Read it before writing, and hold specific passages in mind as calibration (`references/prose-and-proof-standard.md` §1 lists them with PDF page numbers). The following features are what you are replicating — not the thesis's subject matter, its *manner*.
...
Two registers coexist, exactly as in `polymath-notes`: **formal** for definition and theorem *statements*; **thesis-prose** for everything else — motivation, intuition, the unpacking of a definition, the connective tissue of a proof, the section openers. The same two registers, with the same thesis exemplar, govern the rest of the vault; paper notes differ from chapter notes only in their floor-level reader and their backchaining apparatus, never in voice.
...
Reproduce every proof the paper gives, with **no leaps**, at or above the thesis floor (`references/prose-and-proof-standard.md` §6: named assumptions and goal, labelled blocks with bold lead-ins, a justification on every displayed line, every hypothesis invoked by name, all directions and cases, a closing sentence). Expand each "it follows", "clearly", "one checks", "by [X]" into an explicit, justified step. **Every theorem the notes mention is proved — the vault-wide Proof Standard applies to paper notes without exception.** When the paper cites an external result it does not prove, do not silently import it, and do not merely state it: state it and type it in an **external-input callout** (`references/recall-callouts.md`) at the point of use, *and* prove it in full on its own atomic `Thm -`/`Lemma -` page in `Subpages/` (or wikilink an existing vault page whose Formal Proof is complete), linked from the callout. The reader never has to "take it on faith". The only result that may be used without proof is a genuinely book-length one, and only in the `Imported without proof` callout form of the standard's §5, listed in the index page's Verification log. When the paper's own proof has a gap you must fill from your own knowledge, say so (Rule 6) and fill it. A proof in the notes is acceptable only when a floor-level reader could check every line on paper without drawing on anything off the page.
...
`Subpages/Paper - [Short Title] — Whole-Paper Story.md` — a single, coherent, top-to-bottom narrative that walks the *entire* paper as one connected story. **The structural exemplar is `paper_source/example.md`; the sentences are in the thesis register** (`references/prose-and-proof-standard.md` Part I). Read the exemplar before writing the story; the structural standard is not negotiable. What the exemplar shows and what your story must match:
...
### Pass 0 — Read the thesis for style and for the proof floor

Extract and read the reference thesis (`prose/Chiang Sung En-Thesis.pdf`) against `references/prose-and-proof-standard.md`. Internalise the prose features in **The Prose Standard** and the eleven features of a thesis-floor proof (standard §6). Do not skip this even on a re-run — the voice is the deliverable's defining quality, and it is easy to drift back into terse specification register.

### Pass 1 — Read the paper and build the backchain

**Identify the target paper first.** The target is the PDF the user pointed at — in `paper_source/` or `sources/`. The reference thesis lives in `prose/` and is the style exemplar read in Pass 0, never the subject of the notes; if a copy of it sits in `paper_source/`, it is not a candidate either. If more than one non-thesis candidate is present and the user did not name one, ask which paper before proceeding.
...
1. **Re-read `paper_source/example.md` end to end** immediately before writing. That is the structure — one narrative, a picture for every object, a connective sentence at every boundary, a total picture at the end. The sentences themselves are in the thesis register (`references/prose-and-proof-standard.md` Part I).
...
6. **Gap-free proofs at the thesis floor; every theorem mentioned is proved.** Pick two rewritten proofs; every step is a justified move a floor-level reader can check, with no "clearly"/"it follows"/bare "by [X]" left unexpanded, and each proof has the eleven features of `references/prose-and-proof-standard.md` §6. External results appear as external-input callouts with precondition, conclusion, intuition, and source, **and** each is proved in full on its own atomic page (or wikilinks a vault page whose Formal Proof is complete), linked from the callout. No `(Statement)` page exists in the folder; any book-length result used without proof is in the `Imported without proof` callout form and listed in the Verification log. `python3 .claude/skills/polymath-notes/scripts/find-unproved-theorems.py "Study notes/paper/[Short Title]"` is clean.
...
11. **Whole-Paper Story exists, is linked at the top of the index, matches the `paper_source/example.md` structural standard, and is written in the thesis register.** The file `Subpages/Paper - [Short Title] — Whole-Paper Story.md` exists; the index page's first content link (above or immediately after the abstract) points to it with prominent framing ("Whole-paper story — read this first"); and the story matches the example.md exemplar on every dimension named in **What you produce §2** (one flowing narrative in the paper's order, mental picture for every object, connective sentences at every section boundary, symbols only where they earn their place, arguments-at-every-joint rather than stated facts, one-line versions to hold onto scattered where useful, and a final one-paragraph total picture). Coverage: every paper section appears in the story.
