...
6. `references/obsidian-patterns.md` (a symlink to the polymath-notes copy — **the source of truth for Obsidian syntax**: wikilinks, transclusion, collapsible callouts, math delimiters `$...$` / `$$...$$`, never LaTeX inside `[[ ]]`, Windows-portable filenames, YAML frontmatter). When this skill and that file disagree on *syntax*, that file wins.
...
**Intuition is stated before formalism, always.** The thesis never drops a definition cold. It opens with the situation that calls for the object, says in plain words what the object *is* and what it *does*, and only then states it formally — and immediately unpacks the formal statement in a concrete special case. Model passage: the Kullback–Leibler divergence is introduced by first defining entropy as "the average amount of surprise", walking through "excess surprise we experience when we incorrectly assume the distribution is $Q$ but the true distribution is $P$" as a bulleted intuition, and only then writing $D_{KL}(P\|Q) = H(P,Q) - H(P)$. Every definition in your notes follows this shape: *plain-language what-and-why → formal statement → concrete unpacking*.

**Concrete before abstract.** The thesis shows the bivariate case before the general $D$-variable case ("For a bivariate distribution, the Lancaster interaction measure is simply the difference between the joint distribution and the product of the marginals"); it computes Möbius inversion on the three-element chain with the actual $3\times 3$ matrices before invoking the general formula; it enumerates the eight elements of the subset lattice on $\{X,Y,Z\}$ explicitly. When your paper states something in generality, precede it with the smallest concrete instance that makes it obvious.
...
Assume only the undergraduate floor. For **every** concept, notation, or result the paper uses that lives above the floor, close the gap:

- **If a note already exists in the vault** covering that concept, wikilink it and add a one-line reminder of what it says at the point of use (Rule 2's recall). Search the vault first (`grep`/`find` over `Study notes/`) — the vault already has deep Measure Theory, Advanced Probability, Functional-Analysis-flavoured Linear Algebra, Differential Geometry, Topology, Group/Ring/Module theory, and more; reuse it.
- **If no note exists,** create an atomic `Def -`, `Thm -`, or `Lemma -` note for it (template in `references/atomic-note-templates.md`), placed **in its natural subject-area folder** so it cross-links with the study notes — e.g. a Radon–Nikodym note under `Study notes/Probability/Measure Theory/…`, a spectral-theorem note under `Study notes/Analysis/…` or `Study notes/Algebra/Linear Algebra/…`. Create the subject folder if that field is not yet represented. These notes are **vault assets, not paper-local**: they live in the subject hierarchy, not in the paper's folder, because the next paper will reuse them.
- **Then recurse on that note's own prerequisites.** A Radon–Nikodym note needs absolute continuity of measures and the notion of a σ-finite measure; each of those, if above the floor and not yet in the vault, gets its own atomic note, until every leaf bottoms out at the floor. The backchain terminates because the floor is fixed and finite.
...
**For a long paper, split the reading surface** into one page per paper section — `Paper - [Short Title] — §N [Section].md` — with `Paper - [Short Title].md` as a short hub that carries the header, notation, and prerequisites map and links the section pages in order. Split only at the paper's own section boundaries, exactly as `polymath-notes` splits a topic page at sub-chapter boundaries. Default to the single page; split when a single file would become unwieldy.
...
Read the whole paper (extract with `pdftotext -layout`; if the paper is long, extract its table of contents first and read section by section rather than loading it whole — mirror the `polymath-notes` trimmed-PDF workflow). Produce, in `.scratch/`:
...
**For a long paper, commit and push to `main` as you go** — after each section (its atomic notes, its companion section, its stubs), stage and commit with a descriptive message ("Paper notes: [Short Title] §3 — Radon–Nikodym backchain + gap-free proof of Thm 3.1") and push. This preserves work across a long task and is the workflow the user expects for multi-section papers. Follow the vault's git convention (commit after each coherent unit; push per the repository's branch requirements).

### Pass 3 — Low-context-reader pass (leon-proofreader)

After a companion page is drafted, run a low-context-reader pass over it **in the spirit of `leon-proofreader`** (`.claude/skills/synced/leon-proofreader` if present): read the page as someone with exactly the floor and nothing above it, walking top to bottom, and check every place the reader's attention is directed. Specifically for paper notes:
...
Run the self-check below, plus the mechanical audits shared with `polymath-notes` — `find-math-bugs.py`, `find-latex-bugs.py`, `find-wikilink-bugs.py` (paths under `../polymath-notes/scripts/`) — and a wikilink-resolution audit (every `[[ ]]` resolves to a real file; every `![[ ]]` transclusion points to real content). **Do not run `autolinker.py`** over the paper folder — it mislinks paper-local notation. Then report.
...
7. **Verification and flagged uncertainty.** Every definition or lemma supplied from your own knowledge was web-searched and carries a citation. Everything you remained unsure of is flagged with the visible uncertainty marker. The Verification log is present and records what was checked.
