...
1. **The two prose exemplars in `prose/` — the voice and proof-detail baseline for every section page and atomic subpage.** Study *both* closely before writing anything (extract with `pdftotext -layout`, or `pymupdf`/`pypdf` if poppler is absent — `pip install pymupdf` then `import pymupdf`):
   - `prose/Chiang Sung En-Thesis.pdf` — the reference thesis. Governs the *narrative and expository voice*: motivate before you formalise, state the object crisply, then unpack the intuition and the smallest concrete case around it, and write every proof as labelled, justified steps with no leaps.
   - `prose/linear_algebra_done_right.pdf` — Axler, *Linear Algebra Done Right* (4th ed.). Governs how a **named result and its proof** are written: every result carries a descriptive name (not just a number), motivating prose leads into it, and the proof is *complete and self-contained* — every step cites by name the specific prior result, definition, or hypothesis it uses ("by 1.32", "the third condition above implies", "1.34 gives"), so a reader can check each line without supplying anything of their own.

   Together these two set the quality bar. For paper notes they govern the writing voice on section pages and atomic subpages and *supersede* the David-Tong register used elsewhere in the vault. What you replicate from them is specified in **The Prose Standard** below — read that section against both exemplars open beside you. **The thesis's proof detail is the *minimum* baseline: no proof in the notes may be terser or leave more implicit than a thesis proof.**
...
## The Prose Standard — replicate the exemplars in `prose/`

**The two exemplars in `prose/` — the reference thesis (`prose/Chiang Sung En-Thesis.pdf`) and Axler's *Linear Algebra Done Right* (`prose/linear_algebra_done_right.pdf`) — are the voice every paper note must be written in.** Read both before writing, and hold specific passages in mind as calibration. The following features are what you are replicating — not the exemplars' subject matter, their *manner*. The thesis is the model for narrative and for the labelled-step density of a proof; *Linear Algebra Done Right* is the model for how a named result is set up and how a proof cites every step it stands on. They agree on everything below; where a proof is concerned, hold both in mind at once.
...
**Proofs are written as labelled, justified steps — no leaps — and every step names what it stands on.** The thesis proof of V-structure detection consistency splits into "Direction 1" / "Direction 2" and then proceeds by bolded lead-ins — "Apply contrapositive of the contraction axiom:", "Apply d-separation to marginal independence:", "Identify unblocked path through Z:", "Establish Z as collider:", "Conclude V-structure:" — each a single justified move. Its independence computations show every line ($\sum_z P(X,Y,Z) = \sum_z P(X)P(Y)P(Z\mid X,Y) = P(X)P(Y)\sum_z P(Z\mid X,Y) = P(X)P(Y)$). *Linear Algebra Done Right* shows the second half of the discipline: every inference cites the exact prior result, definition, or hypothesis that licenses it — "then $-u$ [which equals $(-1)u$ by 1.32] is also in $U$ by the third condition", "1.34 implies that $V_1 + \cdots + V_m$ is a subspace". Your rewritten proofs combine both: one justified step per line or per bolded move, each step naming the definition/lemma/hypothesis it uses (a `> [!recall]-`, a `> [!cite]-`, or an earlier line), and nothing left to "clearly", "it follows", or a bare "by [X]".

**Proof detail — the non-negotiable floor.** Every proof in the notes is at least as detailed as a thesis proof: the thesis's step density is the *minimum* baseline, never the target to trim toward. A proof that a floor-level reader cannot check line by line, without supplying a step of their own or opening another page, is not finished. If a full proof would run long, it still runs long (Rule 7) — length is never a reason to compress a proof below this floor.
...
### Pass 0 — Read the exemplars for style

Extract and read **both** prose exemplars: the reference thesis (`prose/Chiang Sung En-Thesis.pdf`) and *Linear Algebra Done Right* (`prose/linear_algebra_done_right.pdf`). Internalise the prose features in **The Prose Standard** — the thesis for narrative voice and labelled-step proof density, *Linear Algebra Done Right* for how a named result is motivated and how a proof cites every step it uses. Do not skip this even on a re-run — the voice and the proof-detail floor are the deliverable's defining qualities, and it is easy to drift back into terse specification register.
