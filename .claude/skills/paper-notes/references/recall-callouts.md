...
- **Title = the term, as it appears in the text**, with its notation in parentheses if it has a symbol. Use Unicode inside the title, never LaTeX (Obsidian renders `$...$` literally in a callout *title*; it renders fine in the *body*). So `> [!recall]- σ-algebra`, not `> [!recall]- $\sigma$-algebra`; the body may use `$\sigma$-algebra` freely.
...
> **Statement (typed):** if $\mu, \nu$ are σ-finite measures on $(X, \mathcal{F})$ with $\mu \ll \nu$, then there is a $\nu$-a.e.-unique measurable $f : X \to [0, \infty)$ with $\mu(A) = \int_A f \, d\nu$ for all $A \in \mathcal{F}$. The function $f$ is written $\tfrac{d\mu}{d\nu}$.
...
- **Needs / Says are both symbolic.** The precondition ("Needs") and conclusion ("Says") carry no bare jargon — every term in them is either floor-level or recalled/typed nearby.
